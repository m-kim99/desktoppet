...
### Added (🧺 원거리 휙 담기 — 조종 전용)
- 과일 든 조종 펫 우클릭 시 메뉴 하단에 **"🧺 바구니에 휙 담기"** 행(과일 들었을 때만) —
  바구니까지 안 걸어가도 폴짝 던지기 제스처 + 과일이 **큰 아크로 날아가 바구니에 쏙**(거리
  비례 체공, 회전, 착지 반짝+적립 토스트). 손에서 즉시 사라지고 적립은 착지 순간.
  절친 자율 수거·선물은 그대로 걸어서 배달(사용자 지정). E2E 4종(행 노출·아크 적립·손 비면
  숨김) 추가 — 메뉴 스위트 ALL PASS + 스모크 10/10.
