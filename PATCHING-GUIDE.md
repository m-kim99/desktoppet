...
- 폰 테스트: `sh scripts/serve-phone.sh` — 정적 서버 **0.0.0.0**:8765 바인딩 (루프백 금지). E2E 테스트 서버는 127.0.0.1:8897+ (앱 백엔드 3456과 충돌 금지)
  - ⚠️ **가장 흔한 고장은 죽는 게 아니라 "먹통(wedge)"이다.** 단일스레드 `HTTPServer`는 끊긴
    keep-alive 연결 하나에 전체가 막힌다 — `lsof`엔 LISTEN으로 보이고 맥에서 작은 curl은 통하는데,
    병렬로 수십 개 애셋을 받는 **폰만** 안 붙는다. 몇 주 멀쩡하다가 갑자기 그렇게 된다.
    → 스크립트는 **스레드형**(`python3 -m http.server`, 3.7+ = ThreadingHTTPServer)을 쓴다.
    검증법: 멈춘 연결 3개를 물려 놓고 동시 요청이 200이면 정상
  - ⚠️ 세션 안에서 `... &`로 띄우면 세션이 끝날 때 같이 죽기도 한다 → `nohup`으로 분리(PPID=1 확인)
  - 안 붙을 때 순서: ① `lsof -nP -iTCP:8765 -sTCP:LISTEN`으로 프로세스 생존 ② `*:8765`인지(`127.0.0.1:8765`면 폰 불가) ③ `ipconfig getifaddr en0`로 IP 변동 ④ 그 다음에 코드 의심
