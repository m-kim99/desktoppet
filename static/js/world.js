...
let camUnderwater = false;   // 🤿 카메라가 수면 아래 — updateUnderwater가 판정, 안개·하늘 전환의 단일 신호
const skyCv = document.createElement('canvas');
skyCv.width = 1; skyCv.height = 256;
const skyCtx = skyCv.getContext('2d');
const skyTex = new THREE.CanvasTexture(skyCv);
let skyDomeMesh = null;   // 🤿 수중 모드가 잠깐 숨긴다
{
...
function updateDayNight(force = false) {
    ...
    if (spaceF > 0) {   // 🚀 우주에서 내려다보는 세계 — 안개를 활짝 밀어 군도 전체(R34)가 발밑 미니어처로 (62 = 궤도 카메라에서 최원 섬까지 실측 47 + 여유, draws 예산 배려)
        scene.fog.near = THREE.MathUtils.lerp(scene.fog.near, 14, spaceF);
        scene.fog.far = THREE.MathUtils.lerp(scene.fog.far, 62, spaceF);
    }
    }
...
updateDayNight(true);
// 🚀 카메라 고도 → 우주 블렌드: y7.5(열기구 순항 위)부터 물들기 시작해 13.5에서 만개.
// 변화가 있을 때만 updateDayNight(force) 리페인트 — 상승/하강 전이 몇 초 동안만 캔버스를
// 다시 굽고(≈28회/전이), 궤도 순항·지상에선 0회. 30초 스탬프 스로틀과 공존한다.
function updateSpaceSky() {
    const t = THREE.MathUtils.clamp((camera.position.y - 7.5) / 6.0, 0, 1);
    spaceF = t * t * (3 - 2 * t);
    if (Math.abs(spaceF - spaceFPainted) > 0.035 || ((spaceF === 0 || spaceF === 1) && spaceF !== spaceFPainted)) {
...
// ---- 🚀 발사 플랫폼 상수: 조형·지면 훅·항로 회피·주차 가드가 전부 이걸 읽는다 —
// 이사 = 이 두 줄 + scripts/_rocket-pad-probe.mjs 재실행(페리 항로·섬·탈것·수평선 실간격 검증).
// 본섬 정남 앞바다: 기본 카메라(북→남)의 정면 무대 + 해/달 아크(z=-7 남쪽 하늘)가 발사 배경.
// 북쪽 석호는 페리 진입 레인이 전역을 지나 전 후보 탈락(프로브 실측) — 남쪽은 페리 4.7m·
// 본섬 남안 물길 1.9m(수영 승선)·모험의 섬 3.4m (2026-07-21). ----
...
const world = {
    ...
    groundHeightAt(x, z) {
        ...
        const ry = onRocketPadY(x, z);
        if (ry !== null) return ry;                              // 🚀 발사 플랫폼 갑판도 지면
...
const BED_KO = { sunbed: '선베드', hammock: '해먹', swing: '그네', seesaw: '시소', trampoline: '트램펄린', vine: '덩굴 시렁', fruitbasket: '과일바구니', pond: '연못', portal: '텔레포트', sofa: '소파', loftbed: '2층 침대' };
...
if (statsOn) window.__worldDev = {
    ...
    seaSpawnNow: (id, dx, dz) => {   // E2E — 펫 곁 확정 스폰
        ...
        if (!pp) return false;
        const t = SEAFOOD.find((q) => q.id === id) || SEAFOOD[0];
        const x = pp.mover.position.x + (dx || 0.5), z = pp.mover.position.z + (dz || 0);
        const mesh = makeSeafoodMesh(t.id, 1.5);
        mesh.position.set(x, seabedHeight(x, z) + 0.1, z);
        fruitLayer.add(mesh);
        seaSpawns.push({ t, x, z, mesh });
        return true;
    },
...
diveBtn.onclick = () => {
    if (dive && possessed && dive.p === possessed && !dive.isAI) {
        if (dive.phase === 'deep') { dive.phase = 'up'; dive.t = 0; }   // ⬆️ 부상
    } else if (possessed && possessed.swimming === 'sea') startDive(possessed);
};
function syncDiveBtn() {   // 🤿 문맥 버튼: 바다 수영=🤿, 잠수 중=⬆️ (매 프레임 — 변경 가드로 무비용)
    const diving = !!(dive && possessed && dive.p === possessed && !dive.isAI);
    const show = !!(possessed && (possessed.swimming === 'sea' || diving));
    const disp = show ? 'flex' : 'none';
    if (diveBtn.style.display !== disp) diveBtn.style.display = disp;
    if (show) {
        const want = diving ? '⬆️' : '🤿';
        if (diveBtn.textContent !== want) {
            diveBtn.textContent = want;
...
const PROP_LABELS = { tree: '나무', bowl: '밥그릇', fence: '울타리', sunbed: '선베드', hammock: '해먹', lamp: '가로등', radio: '라디오', coffee: '커피 부스', food: '간식 부스', swing: '그네', seesaw: '시소', trampoline: '트램펄린', vine: '덩굴 시렁', fruitbasket: '과일바구니', pond: '연못', portal: '텔레포트', house: '집', car: '자동차' };
...
function makeFoodGeo(f, bites = 0) {
    ...
    if (f.id === 'toast') {                                   // 우상단 모서리를 베어문다 — 이빨 자국 스캘럽 실루엣 + 크럼 단면색
        ...
        const xL = b >= 2 ? 0.015 : b >= 1 ? -0.008 : -0.04, L = 0.04 - xL, cx = (xL + 0.04) / 2;
        ...
        topH = 0.05;
    } else if (f.id === 'hotdog') {                           // 끝부터 베어문다 — 짧아지고 단면에 빵 링+소시지 심
        const xL = b >= 2 ? 0.018 : b >= 1 ? -0.014 : -0.046;
        const bunL = 0.046 - xL, bcx = (xL + 0.046) / 2;
        const bun = new THREE.CylinderGeometry(0.021, 0.021, bunL, 16); bun.rotateZ(Math.PI / 2); bun.translate(bcx, 0.026, 0); add(bun, 0xe9c187, 0xcf9f57, { curve: 1.14 });
        const bcR = new THREE.SphereGeometry(0.021, 14, 11); bcR.translate(0.046, 0.026, 0); add(bcR, 0xe9c187, 0xcf9f57, { curve: 1.14 });
        if (!b) { const bcL = new THREE.SphereGeometry(0.021, 14, 11); bcL.translate(-0.046, 0.026, 0); add(bcL, 0xe9c187, 0xcf9f57, { curve: 1.14 }); }
        const sxR = 0.05, sxL2 = Math.max(xL, -0.05), sL = sxR - sxL2, scx = (sxL2 + sxR) / 2;
        const sau = new THREE.CylinderGeometry(0.0125, 0.0125, sL, 14); sau.rotateZ(Math.PI / 2); sau.translate(scx, 0.045, 0); add(sau, 0xda6150, 0xb5403a, { curve: 1.25 });
        const seR = new THREE.SphereGeometry(0.0125, 12, 9); seR.translate(0.05, 0.045, 0); add(seR, 0xda6150, 0xb5403a, { curve: 1.25 });
        if (!b) { const seL = new THREE.SphereGeometry(0.0125, 12, 9); seL.translate(-0.05, 0.045, 0); add(seL, 0xda6150, 0xb5403a, { curve: 1.25 }); }
        else { csFace(0.021, 16, xL, 0.026, 0, 0xecd9b0, 0xdcc290); csFace(0.0125, 14, xL - 0.0004, 0.045, 0, 0xe07a5a, 0xc85a3e); }
        for (let i = 0; i < 4; i++) { const mx = -0.031 + i * 0.021; if (mx > xL + 0.01) { const my = new THREE.TorusGeometry(0.011, 0.0026, 6, 9, Math.PI); my.rotateX(Math.PI / 2 - 0.5); my.rotateZ(0.6); my.translate(mx, 0.056, 0.004); add(my, 0xeeb833, 0xd0901f, { curve: 1 }); } }
        topH = 0.058;
    } else if (f.id === 'donut') {                            // 링을 베어문다 — 부분 링(열린 C) + 물린 끝 도우
        const full = Math.PI * 2, arc = b >= 2 ? full * 0.6 : b >= 1 ? full * 0.8 : full;
        const a0 = b ? Math.PI / 2 + (full - arc) / 2 : 0, seg = Math.max(7, Math.round(24 * arc / full));
        const ring = new THREE.TorusGeometry(0.03, 0.015, 14, seg, arc); ring.rotateZ(a0); ring.rotateX(0.5); ring.translate(0, 0.032, 0); add(ring, 0xf6a9c0, 0xe07fa0, { curve: 1.15 });
        const glaze = new THREE.TorusGeometry(0.03, 0.0135, 14, seg, arc); glaze.scale(1, 1, 0.62); glaze.rotateZ(a0); glaze.rotateX(0.5); glaze.translate(0, 0.034, 0.006); add(glaze, 0xfcc4d6, 0xf3a6bf, { curve: 1 });
        if (b) { for (const ae of [a0, a0 + arc]) { const cap = new THREE.SphereGeometry(0.014, 10, 8); cap.translate(Math.cos(ae) * 0.03, Math.sin(ae) * 0.03, 0); cap.rotateX(0.5); cap.translate(0, 0.032, 0); add(cap, 0xefd7a6, 0xe0c184, { curve: 1 }); } }
        const spc = [0xe0473d, 0x5aa9e0, 0xe5a72c, 0x6bbf6a, 0xf3ece2], nspr = b >= 2 ? 6 : b >= 1 ? 8 : 10;
        ...
        if (b >= 2) {
            const cake = new THREE.SphereGeometry(0.031, 14, 11); cake.scale(1, 0.66, 1); cake.translate(0, 0.046, 0);
            addB(cake, 0xf0d9a8, 0xe0bd7e, { curve: 1.1 }, { cy: 0.046, ry: 0.03, ft: 0xf7e6c2, fb: 0xecd39a }, [{ x: 0.012, y: 0.064, z: 0.014, r: 0.028 }]);
            for (let i = 0; i < 4; i++) { const cr = new THREE.SphereGeometry(0.0035, 5, 4); cr.translate(Math.cos(i * 1.9) * 0.02, 0.05 + (i % 2) * 0.006, Math.sin(i * 1.9) * 0.02); add(cr, 0xf3dca8, 0xe6c98a, { curve: 1 }); }
        } else if (b >= 1) {
            const s1 = new THREE.SphereGeometry(0.031, 16, 12); s1.scale(1, 0.72, 1); s1.translate(0, 0.05, 0);
            addB(s1, 0xf9b2c7, 0xe888a8, { curve: 1.25 }, { cy: 0.05, ry: 0.03, ft: 0xf0d9a8, fb: 0xe0bd7e }, [{ x: 0.012, y: 0.074, z: 0.012, r: 0.024 }]);
            const s2 = new THREE.SphereGeometry(0.02, 14, 11); s2.scale(1, 0.8, 1); s2.translate(-0.006, 0.064, -0.004); add(s2, 0xfbbdd1, 0xef95b2, { curve: 1.25 });
            const cake = new THREE.SphereGeometry(0.013, 12, 9); cake.translate(0.014, 0.076, 0.012); add(cake, 0xf0d9a8, 0xe0bd7e, { curve: 1.1 });
            const csp = [0xe0473d, 0x3f9ae0, 0xf2c81f, 0x54bf54, 0xb85ad8];
            for (let i = 0; i < 6; i++) { const a = i * 2.4; const rr = 0.03 - (i % 3) * 0.005; const sp = new THREE.BoxGeometry(0.012, 0.004, 0.004); sp.rotateZ(a * 1.6); sp.rotateY(a); sp.translate(Math.cos(a) * rr, 0.055 + (i % 3) * 0.007, Math.sin(a) * rr); add(sp, csp[i % 5], csp[i % 5], { curve: 1 }); }
        } else {
            const s1 = new THREE.SphereGeometry(0.031, 16, 12); s1.scale(1, 0.72, 1); s1.translate(0, 0.05, 0); add(s1, 0xf9b2c7, 0xe888a8, { curve: 1.25 });
            const s2 = new THREE.SphereGeometry(0.023, 14, 11); s2.scale(1, 0.82, 1); s2.translate(0.004, 0.066, 0.003); add(s2, 0xfbbdd1, 0xef95b2, { curve: 1.25 });
            const s3 = new THREE.SphereGeometry(0.015, 12, 10); s3.translate(-0.003, 0.08, 0); add(s3, 0xfdcad9, 0xf3a3c0, { curve: 1.25 });
            const csp = [0xe0473d, 0x3f9ae0, 0xf2c81f, 0x54bf54, 0xffffff, 0xb85ad8];
            for (let i = 0; i < 11; i++) { const a = i * 2.4; const yy = 0.052 + (i % 4) * 0.008; const rr = 0.033 - (yy - 0.052) * 0.62; const sp = new THREE.BoxGeometry(0.012, 0.004, 0.004); sp.rotateZ(a * 1.6); sp.rotateY(a); sp.translate(Math.cos(a) * rr, yy, Math.sin(a) * rr); add(sp, csp[i % 6], csp[i % 6], { curve: 1 }); }
            const cherry = new THREE.SphereGeometry(0.0095, 12, 10); cherry.translate(0, 0.094, 0); add(cherry, 0xe8514a, 0xc0342f, { curve: 1.2 });
        }
        topH = 0.08;
    }
...
// ☕🥤 음료 9종 — 과일·음식과 같은 규율(전 파트 bakeGrad 정점색 → 병합). sips 0=가득 1=절반 2=조금,
...
function makeDrinkGeos(d, sips = 0) {
    ...
    if (d.iced) {                                             // ── 아이스 4종: 투명컵 + 수위 + 얼음 + 빨대
        const IL = [0.062, 0.038, 0.014], yT = IL[s];
        const rAt = (y) => 0.021 + (y / 0.075) * 0.007, rT = rAt(yT) - 0.0012, rB = 0.0198;
        glass = new THREE.CylinderGeometry(0.028, 0.021, 0.075, 20, 1, true); glass.translate(0, 0.0375, 0);
        const cupBase = new THREE.CylinderGeometry(0.0212, 0.0212, 0.004, 20); cupBase.translate(0, 0.002, 0); add(cupBase, 0xe6f2f9, 0xc6dbe6, { curve: 1 });
        const rim = new THREE.TorusGeometry(0.0276, 0.0019, 6, 22); rim.rotateX(Math.PI / 2); rim.translate(0, 0.0748, 0); add(rim, 0xeef6fb, 0xcfe0ea, { curve: 1 });
        const layered = d.id === 'strawberry' || d.id === 'matcha';
        const milk = d.id === 'strawberry' ? 0xfdf6f2 : 0xfbf8f0;
        if (layered && s < 2) {                               // 2층 — 아래 우유 · 위 음료
            const yM = s === 0 ? 0.030 : 0.022, rM = rAt(yM) - 0.0012;
            const lo = new THREE.CylinderGeometry(rM, rB, yM - 0.004, 20); lo.translate(0, (yM + 0.004) / 2, 0); add(lo, milk, shade(milk, -14), { curve: 1 });
            const hi = new THREE.CylinderGeometry(rT, rM, yT - yM, 20); hi.translate(0, (yT + yM) / 2, 0); addLiq(hi, shade(base, 14), base, { curve: 1.1 });
        } else {
        ...
        for (const [cx, cz, rot] of cubes) {
            const ice = new THREE.BoxGeometry(0.0105, 0.0095, 0.0105); ice.rotateY(rot); ice.rotateZ(rot * 0.35);
            ice.translate(cx, s >= 2 ? 0.0105 : yT + 0.0022, cz); add(ice, 0xeaf7ff, 0xc4dcea, { curve: 1 });
        }
        if (d.id === 'strawberry') for (let i = 0; i < 5; i++) { const a = i * 1.9, rr = 0.010 + (i % 2) * 0.005; const pl = new THREE.SphereGeometry(0.0024, 6, 5); pl.translate(Math.cos(a) * rr, yT + 0.0012, Math.sin(a) * rr); add(pl, 0xe0526f, 0xb63450, { curve: 1 }); }
        if (d.id === 'matcha') for (let i = 0; i < 5; i++) { const a = i * 2.2, rr = 0.009 + (i % 2) * 0.005; const pw = new THREE.SphereGeometry(0.0021, 5, 4); pw.translate(Math.cos(a) * rr, yT + 0.0011, Math.sin(a) * rr); add(pw, 0x6e9c46, 0x527a31, { curve: 1 }); }
        if (d.id === 'icetea') {                              // 레몬 슬라이스 — 수면에 걸쳐 뜬다
            const ly = (s >= 2 ? 0.0125 : yT + 0.0022);
            const lem = new THREE.CylinderGeometry(0.0098, 0.0098, 0.0026, 16); lem.rotateX(Math.PI / 2); lem.rotateZ(0.34); lem.translate(0.0145, ly, 0.0105); add(lem, 0xffe07a, 0xefc648, { curve: 1 });
            const lin = new THREE.CylinderGeometry(0.007, 0.007, 0.0034, 16); lin.rotateX(Math.PI / 2); lin.rotateZ(0.34); lin.translate(0.0145, ly, 0.0105); add(lin, 0xfff6c9, 0xffe999, { curve: 1 });
        }
        const stc = d.id === 'strawberry' ? 0xf26d8f : d.id === 'matcha' ? 0x7fb04a : d.id === 'icetea' ? 0xe8734a : 0xef8a8a;
        const straw = new THREE.CylinderGeometry(0.0033, 0.0033, 0.086, 8); straw.rotateZ(-0.2); straw.translate(0.0105, 0.056, 0.0015);
        add(straw, shade(stc, 22), shade(stc, -22), { curve: 1 });
        ...
        if (d.id === 'cappuccino') {                          // 우유 거품 — 림 위로 봉긋, 줄면 얇은 막
            ...
            const foam = new THREE.SphereGeometry(lr * 1.02, 16, 11); foam.scale(1, fh, 1); foam.translate(0, ly + 0.0035, 0); add(foam, 0xfffdf7, 0xf0e9db, { curve: 1.1 });
            for (let i = 0; i < 4; i++) { const a = i * 1.75; const dot = new THREE.SphereGeometry(0.0014, 5, 4); dot.translate(Math.cos(a) * lr * 0.46, ly + 0.0035 + lr * fh * 0.82, Math.sin(a) * lr * 0.46); add(dot, 0x9a6a3c, 0x774f28, { curve: 1 }); }
        }
        if (d.id === 'choco') {                               // 휘핑크림 소용돌이 + 코코아
            const n = s === 2 ? 1 : s === 1 ? 2 : 3;
            for (let i = 0; i < n; i++) { const rr3 = lr * (0.88 - i * 0.24); const sw = new THREE.SphereGeometry(rr3, 13, 9); sw.scale(1, 0.5, 1); sw.translate(0.0009 * i, ly + 0.0035 + i * rr3 * 0.52, 0.0006 * i); add(sw, 0xfffaf1, 0xefe1cd, { curve: 1.15 }); }
            for (let i = 0; i < 3; i++) { const a = i * 2.2; const dot = new THREE.SphereGeometry(0.0014, 5, 4); dot.translate(Math.cos(a) * lr * 0.4, ly + 0.006 + lr * 0.3, Math.sin(a) * lr * 0.4); add(dot, 0x7a4a28, 0x5c3418, { curve: 1 }); }
        }
        topH = 0.062;
    }
    const mg = (arr) => (arr.length ? mergeGeometries(arr.map((p) => (p.index ? p.toNonIndexed() : p)), false) : null);
    return { opaque: mg(op), glass, liquid: mg(tr), liquidSoft: mg(ts), topH };
}
function makeDrinkMesh(d) {
    ...
    if (g.liquidSoft) grp.add(new THREE.Mesh(g.liquidSoft, liquidSoftMat));
...
function updateDrinkGeo(it, sips) {   // 한 모금마다 수위·얼음·거품이 실제로 줄어든 형상으로 교체
    ...
    let i = 0;
    for (const geo of [g.opaque, g.glass, g.liquid, g.liquidSoft]) { if (!geo) continue; if (kids[i]) { kids[i].geometry.dispose(); kids[i].geometry = geo; } i++; }
}
...
function giveDrink(p, d) {
    ...
    mesh.rotation.y = Math.PI;   // 과일·음식과 동일 — 앞면(라떼아트·레몬·빨대)이 시청자 쪽으로
    p.pet.wrap.add(mesh);
...
function updateHeldPose(p, key, delta) {
    ...
    if (it.seq) {
        ...
        if (idx !== it.seq.played && it.seq.t < it.seq.count * per) {
            it.seq.played = idx;
            if (isDrink) {
                // 한 번의 "마시기"=정확히 한 단계 (시퀀스 첫 꿀꺽 idx 0에서만 형상 교체) — 나머지는 소리 연출.
                if (idx === 0) { it.gulps += 1; updateDrinkGeo(it, Math.min(2, it.gulps)); }
                playBuffer(sipBuf, { vol: 0.55, rate: 1.05 + Math.random() * 0.25, filterFreq: 620 });
            } else {
                // 과일·음식 모두 한 번의 "먹기"=정확히 한 입 단계 (시퀀스 첫 씹기 idx 0에서만 형상 교체).
                // 나머지 씹기 비트는 우적우적 소리·머리 까딱임만(연출). 1입→2입→(3입=소멸) 3단계.
                if (idx === 0) {
                    it.bites += 1;
        ...
        if (it.seq.t >= total) {
            it.seq = null;
            const finished = isDrink ? it.gulps >= 3 : it.bites >= 3;   // 음료 3모금 · 과일/음식 3입 (가득·절반·조금 → 소멸)
            if (finished) {
...
function releasePossession() {
    ...
    if (rocketRide && (rocketRide.p === p || rocketRide.friend === p)) exitRocketForce(); // 🚀 갑판/물가로, 빈 로켓은 여정 완주
    if (p.poiWalk) poiAutoReturn();   // 🧑‍🚀 달·정거장 산책 중 해제 — 펫들이 알아서 로켓 타고 귀환
...
function doInteract() {
    ...
    if (ROCKET.mode === 'parked' && !rocketRide
        && Math.hypot(possessed.mover.position.x - ROCKET_PAD.x, possessed.mover.position.z - ROCKET_PAD.z) < 1.7) {
        enterRocket(possessed);   // 🚀 발사 플랫폼 갑판 — 캐빈으로 (절친이 곁에 있으면 함께)
        return;
    }
...
function playerSupportY(p, x, z) {
    if (p.poiWalk) return { y: poiSurfaceY(p.poiWalk, x, z), medium: 'land' };   // 🧑‍🚀 EVA — 펫 플래그 게이트 (전역 지면 훅 오염 금지)
    let base;
    if (Math.hypot(x - pondPropRef.x, z - pondPropRef.z) < 0.55) {
        ...
        const ry = hf === null && !hit && ty === null ? onRocketPadY(x, z) : null;
        ...
        else if (ry !== null) base = { y: ry, medium: 'land' };   // 🚀 발사 플랫폼 갑판 (같은 문법 — 훅 2곳 필수)
        else {
...
function nearestClimbSpot(pos) {
    {   // 🚀 발사 플랫폼 림 — 헤엄쳐 와서 갑판으로 폴짝 (섬 림과 같은 문법)
        const dx = pos.x - ROCKET_PAD.x, dz = pos.z - ROCKET_PAD.z;
        const rr = Math.hypot(dx, dz);
        if (rr >= ROCKET_PAD_R - 0.05 && rr < ROCKET_PAD_R + 0.6) {
            const k = (ROCKET_PAD_R - 0.45) / (rr || 1);
            return { tx: ROCKET_PAD.x + dx * k, tz: ROCKET_PAD.z + dz * k, ty: ROCKET_PAD_DECK_Y };
        }
    }
    for (const s of ISLANDS) {
...
function updatePlayer(delta) {
    ...
    if (airborne) {
        jumpVy -= (p.poiWalk && p.poiWalk.id === 'moon' ? 2.6 : 7.0) * delta;   // 🌕 달은 저중력 — 붕 떠오르는 문워크 점프
...
function snapToLand(p) {
    const pos = p.mover.position;
    if (p.poiWalk) return;   // 🧑‍🚀 달·정거장 산책 중 — poiAutoReturn이 처리 (섬 순간이동 금지)
    if (!p.swimming && onRocketPadY(pos.x, pos.z) !== null) { pos.y = ROCKET_PAD_DECK_Y; return; }   // 🚀 갑판 위 해제는 그 자리 (섬으로 순간이동 방지)
...
function boatBlocked(nx, nz) {
    ...
    if (Math.hypot(nx - ROCKET_PAD.x, nz - ROCKET_PAD.z) < ROCKET_PAD_R + 0.35) return true;   // 🚀 발사 플랫폼 선체
...
{   // 🔨 저장된 주차 위치 (plane-1) — 수륙양용이라 뭍·물 어디든 유효, 월드 경계 밖만 무시
    ...
    if (o && Number.isFinite(o.x) && Number.isFinite(o.z) && Math.hypot(o.x, o.z) <= EXPLORE_R
        ...
        && Math.hypot(o.x - ROCKET_PAD.x, o.z - ROCKET_PAD.z) > ROCKET_PAD_R + 0.7) {   // 🚀 발사 플랫폼 위 주차 저장 무시
...
let rocketRide = null;   // { p, friend, t, empty } — empty = 승객 없이 남은 여정 완주(강제 하차)
// 제트 파티클 (셰이더 Points): prog = fract(uWxT·speed + phase)를 수명으로 — 위/아래로 뻗고
// 바깥으로 벌어지며 알파가 사그라든다. 비/눈(precipPoints)과 같은 원리, 스폰 링·방향만 다르다.
const flameTex = precipTexture((ctx) => {
    const g = ctx.createRadialGradient(16, 16, 0, 16, 16, 15);
    g.addColorStop(0, 'rgba(255,246,214,1)');
    g.addColorStop(0.35, 'rgba(255,183,92,0.95)');
    g.addColorStop(0.7, 'rgba(240,96,52,0.55)');
    g.addColorStop(1, 'rgba(240,96,52,0)');
    ctx.fillStyle = g;
    ctx.beginPath(); ctx.arc(16, 16, 15, 0, Math.PI * 2); ctx.fill();
});
const steamTex = precipTexture((ctx) => {
    const g = ctx.createRadialGradient(16, 16, 0, 16, 16, 15);
    g.addColorStop(0, 'rgba(248,246,240,0.9)');
    g.addColorStop(0.6, 'rgba(238,236,230,0.5)');
    g.addColorStop(1, 'rgba(238,236,230,0)');
    ctx.fillStyle = g;
    ctx.beginPath(); ctx.arc(16, 16, 15, 0, Math.PI * 2); ctx.fill();
});
const smokeTex = precipTexture((ctx) => {
    const g = ctx.createRadialGradient(16, 16, 0, 16, 16, 15);
    g.addColorStop(0, 'rgba(230,226,220,0.85)');
    g.addColorStop(0.55, 'rgba(212,208,202,0.5)');
    g.addColorStop(1, 'rgba(212,208,202,0)');
    ctx.fillStyle = g;
    ctx.beginPath(); ctx.arc(16, 16, 15, 0, Math.PI * 2); ctx.fill();
});
function jetPoints(count, tex, size, len, spread, upDir, speedLo, speedHi, { ringR = 0, additive = false, grow = 0 } = {}) {
    const pos = new Float32Array(count * 3), spd = new Float32Array(count), ph = new Float32Array(count), dir = new Float32Array(count * 2);
    for (let i = 0; i < count; i++) {
        const a = Math.random() * Math.PI * 2, rr = ringR * (0.5 + Math.random() * 0.5);
        pos[i * 3] = Math.cos(a) * rr;
        pos[i * 3 + 1] = 0;
        pos[i * 3 + 2] = Math.sin(a) * rr;
        spd[i] = speedLo + Math.random() * (speedHi - speedLo);
        ph[i] = Math.random();
        const da = ringR > 0 ? a : Math.random() * Math.PI * 2;   // 링 스폰은 바깥으로, 축 스폰은 사방으로
        const dr = 0.3 + Math.random() * 0.7;
        dir[i * 2] = Math.cos(da) * dr;
        dir[i * 2 + 1] = Math.sin(da) * dr;
    }
    const geo = new THREE.BufferGeometry();
    geo.setAttribute('position', new THREE.BufferAttribute(pos, 3));
    geo.setAttribute('aSpeed', new THREE.BufferAttribute(spd, 1));
    ...
    const mat = new THREE.PointsMaterial({
        map: tex, size, transparent: true, opacity: 0, depthWrite: false,
    ...
    mat.onBeforeCompile = (sh) => {
        ...
        sh.vertexShader = 'uniform float uWxT;\nattribute float aSpeed;\nattribute float aPhase;\nattribute vec2 aDir;\nvarying float vLife;\n' + sh.vertexShader.replace(
            '#include <begin_vertex>',
            '#include <begin_vertex>\n'
            + 'float prog = fract(uWxT * aSpeed + aPhase);\n'
            + 'vLife = 1.0 - prog;\n'
            + `transformed.y += ${upDir > 0 ? '' : '-'}prog * ${len.toFixed(2)};\n`
            + `transformed.x += aDir.x * prog * ${spread.toFixed(2)};\n`
            + `transformed.z += aDir.y * prog * ${spread.toFixed(2)};`
        );
        // grow: 나이 들수록 점이 커진다 (뭉게뭉게 — 연기/증기용). prog는 begin_vertex에서 이미 정의됨
        if (grow) sh.vertexShader = sh.vertexShader.replace('gl_PointSize = size;', `gl_PointSize = size * ( ${Math.max(0.15, 1 - grow * 0.45).toFixed(2)} + prog * ${grow.toFixed(2)} );`);
        sh.fragmentShader = 'varying float vLife;\n' + sh.fragmentShader.replace(
            '#include <color_fragment>',
            '#include <color_fragment>\ndiffuseColor.a *= vLife;'
        );
    };
    const pts = new THREE.Points(geo, mat);
    pts.frustumCulled = false;   // 셰이더가 정적 바운드 밖으로 밀어낸다
    pts.visible = false;
    return pts;
}
function makeRocket() {
    ...
    finShape.moveTo(0.22, 0.06);
    finShape.lineTo(0.26, 0.66);
    finShape.quadraticCurveTo(0.44, 0.52, 0.52, 0.24);
    ...
    const bodyPts = [
        ...
        new THREE.Vector2(0.295, 0.50), new THREE.Vector2(0.30, 0.62), new THREE.Vector2(0.29, 0.78),
    ...
    dark.push(consolePost);
    for (const ry of [0.985, 1.235]) {
        const rimT = new THREE.TorusGeometry(0.239, 0.013, 8, 24).rotateX(Math.PI / 2);
    ...
    metal.push(mast);
    // 허리 레드 밴드 + 현창 2 (앞뒤)
    const band = new THREE.TorusGeometry(0.302, 0.016, 8, 26).rotateX(Math.PI / 2);
    band.translate(0, 0.52, 0);
    red.push(band);
    for (const [py, pz] of [[0.42, 0.286], [0.66, -0.298]]) {
        const rim = new THREE.TorusGeometry(0.055, 0.013, 8, 16);
        if (pz < 0) rim.rotateY(Math.PI);
        rim.translate(0, py, pz);
        metal.push(rim);
        const glass = new THREE.CircleGeometry(0.047, 12);
        if (pz < 0) glass.rotateY(Math.PI);
    ...
    // 캐빈 실내 웜 라이닝: 벽(BackSide — 어느 각도에서든 펫 "뒤"만 깔림) + 바닥(윗면) + 천장(아랫면).
    // 빛 무시(Basic)라 밤·우주에선 "실내등 켜진 캐빈"이 공짜 — 어두운 우주에서 펫 실루엣이 산다.
    const backdrop = new THREE.Mesh(
        new THREE.CylinderGeometry(0.228, 0.228, 0.25, 20, 1, true),
        new THREE.MeshBasicMaterial({ color: 0xffe9c6, side: THREE.BackSide })
    );
    backdrop.position.y = 1.105;
    g.add(backdrop);
    const cabinFloorGeo = new THREE.CircleGeometry(0.235, 20).rotateX(-Math.PI / 2);
    cabinFloorGeo.translate(0, 0.965, 0);
    const cabinCeilGeo = new THREE.CircleGeometry(0.235, 20).rotateX(Math.PI / 2);
    cabinCeilGeo.translate(0, 1.233, 0);
    const cabinWarm = new THREE.Mesh(mergeGeometries([cabinFloorGeo, cabinCeilGeo], false), new THREE.MeshBasicMaterial({ color: 0xf6dfb8 }));
    g.add(cabinWarm);
    ...
    g.add(glassBand);
    g.traverse((o) => { if (o.isMesh) { o.castShadow = true; o.receiveShadow = true; } });
    for (const o of [backdrop, cabinWarm]) { o.castShadow = false; o.receiveShadow = false; }
    // 라이브 파츠: 비콘(점멸)·화염 코어 2중 콘(가산·꼭짓점 아래)·화염 제트·연기 트레일 — 베이크 밖, 그림자 없음
    const beaconMat = new THREE.MeshBasicMaterial({ color: 0xff8c7a, transparent: true, opacity: 0.9 });
    const beacon = new THREE.Mesh(new THREE.SphereGeometry(0.026, 8, 6), beaconMat);
    beacon.position.y = 1.675;
    g.add(beacon);
    const coreMat = new THREE.MeshBasicMaterial({ color: 0xfff3cd, transparent: true, opacity: 0, blending: THREE.AdditiveBlending, depthWrite: false, side: THREE.DoubleSide });
    const core = new THREE.Mesh(new THREE.ConeGeometry(0.075, 0.62, 12, 1, true), coreMat);
    core.rotation.x = Math.PI;
    core.position.y = -0.2;
    core.visible = false;
    g.add(core);
    const outerMat = new THREE.MeshBasicMaterial({ color: 0xff9e4e, transparent: true, opacity: 0, blending: THREE.AdditiveBlending, depthWrite: false, side: THREE.DoubleSide });
    const outer = new THREE.Mesh(new THREE.ConeGeometry(0.145, 0.95, 12, 1, true), outerMat);
    outer.rotation.x = Math.PI;
    outer.position.y = -0.33;
    outer.visible = false;
    g.add(outer);
    const flamePts = jetPoints(140, flameTex, 0.6, 1.7, 0.32, -1, 1.1, 2.0, { additive: true });   // Points 크기는 그룹 스케일을 안 탄다 — 월드 단위 실측(0.85는 동체를 삼킴, 0.4는 색종이)
    flamePts.position.y = 0.12;
    g.add(flamePts);
    const smokePts = jetPoints(90, smokeTex, 0.95, 2.4, 0.9, -1, 0.35, 0.7, { grow: 1.8 });
    smokePts.position.y = 0.06;
    g.add(smokePts);
    g.userData = { beaconMat, coreMat, core, outerMat, outer, flameMat: flamePts.material, flamePts, smokeMat: smokePts.material, smokePts };
    return g;
}
const RK_SCALE = 2.4;   // 펫 대비 체급 — 1.75에선 혼자 타도 낀 그림(사용자 리포트). 캐빈 내경 1.13m(둘 등맞대기 여유), 좌석·패드 수치는 이 스케일 기준
const rocketGroup = makeRocket();
rocketGroup.scale.setScalar(RK_SCALE);
rocketGroup.position.set(ROCKET.x, ROCKET.y, ROCKET.z);
rocketGroup.rotation.y = ROCKET.heading;
stage.add(rocketGroup);
// 갠트리 타워는 패드 "측면"(동쪽 접선 방향) — 광장 반대편에 두면 기본 카메라(북→남)에서
// 가로대가 로켓 실루엣을 관통해 보인다 (스샷 실측). 측면이면 나란히 서는 그림.
const ROCKET_TWR_UX = -ROCKET_PAD_UZ, ROCKET_TWR_UZ = ROCKET_PAD_UX;
const ROCKET_TWR_X = ROCKET_PAD.x + ROCKET_TWR_UX * (ROCKET_PAD_R - 0.5);
const ROCKET_TWR_Z = ROCKET_PAD.z + ROCKET_TWR_UZ * (ROCKET_PAD_R - 0.5);
{   // 발사 플랫폼 (정적부): 바지선 헐 + 나무 갑판 + 레드 트림/발사 마킹 + 클리트 + 트러스 갠트리
    // — WORLD_STATIC_ROOTS로 월드 베이크 편입 (재질 버킷이 전역 병합돼 드로우 잔돈이 안 는다)
    const st = new THREE.Group();
    const px = ROCKET_PAD.x, pz = ROCKET_PAD.z;
    const hullGeo = new THREE.CylinderGeometry(ROCKET_PAD_R, ROCKET_PAD_R - 0.16, 0.36, 30);
    hullGeo.translate(px, ROCKET_PAD_DECK_Y - 0.185, pz);
    st.add(new THREE.Mesh(bakeGrad(hullGeo, 0xd9d3c6, 0x82868c, { curve: 1.3 }), gradMat));
    const deckGeo = new THREE.CylinderGeometry(ROCKET_PAD_R - 0.06, ROCKET_PAD_R - 0.06, 0.025, 30);
    deckGeo.translate(px, ROCKET_PAD_DECK_Y - 0.012, pz);
    st.add(new THREE.Mesh(deckGeo, M(0xb08a60, { map: plankTex })));
    const redG = [];
    const trimGeo = new THREE.TorusGeometry(ROCKET_PAD_R - 0.045, 0.03, 8, 34).rotateX(Math.PI / 2);
    trimGeo.translate(px, ROCKET_PAD_DECK_Y - 0.002, pz);
    redG.push(trimGeo);
    const markGeo = new THREE.RingGeometry(1.32, 1.45, 30).rotateX(-Math.PI / 2);   // 2.4× 로켓 핀(r≈1.25) 바깥
    markGeo.translate(px, ROCKET_PAD_DECK_Y + 0.004, pz);
    redG.push(markGeo);
    const towerLight = new THREE.SphereGeometry(0.04, 8, 6);   // 타워 꼭대기 경고등 — 타이바 위에 얹힌다 (정적 — 점멸은 로켓 비콘 몫)
    towerLight.translate(ROCKET_TWR_X, ROCKET_PAD_DECK_Y + 3.34, ROCKET_TWR_Z);
    redG.push(towerLight);
    {
        const merged = mergeGeometries(redG.map((q) => (q.index ? q.toNonIndexed() : q)), false);
        if (merged) st.add(new THREE.Mesh(merged, M(0xd05a4a)));
    }
    const metalG = [];
    for (const a of [0.6, 2.2, 3.8, 5.4]) {   // 계선 클리트 4
        const c = new THREE.BoxGeometry(0.08, 0.05, 0.05);
        c.translate(px + Math.cos(a) * (ROCKET_PAD_R - 0.16), ROCKET_PAD_DECK_Y + 0.025, pz + Math.sin(a) * (ROCKET_PAD_R - 0.16));
        metalG.push(c);
    }
    // 트러스 갠트리: 다리 2 + 가로대 4 + X 브레이스 + 캐빈 높이 정비 플랫폼 (3.8m 로켓 체급).
    // 다리 벌린 축 = 접선(TWR_U, 동서) — 기본 카메라(북→남)와 직교라 트러스 실루엣이 제대로 읽힌다
    const aU = Math.atan2(ROCKET_TWR_UX, ROCKET_TWR_UZ);
    for (const s of [-1, 1]) {
        const leg = new THREE.CylinderGeometry(0.045, 0.055, 3.3, 8);
        leg.translate(ROCKET_TWR_X + ROCKET_TWR_UX * s * 0.19, ROCKET_PAD_DECK_Y + 1.65, ROCKET_TWR_Z + ROCKET_TWR_UZ * s * 0.19);
        metalG.push(leg);
    }
    for (const hy of [0.55, 1.25, 1.95, 2.65, 3.28]) {   // 3.28 = 꼭대기 타이바 (경고등 받침)
        const bar = new THREE.BoxGeometry(0.05, 0.045, 0.46);
        bar.rotateY(aU);
        bar.translate(ROCKET_TWR_X, ROCKET_PAD_DECK_Y + hy, ROCKET_TWR_Z);
        metalG.push(bar);
    }
    for (const [tilt, hy] of [[0.63, 0.9], [-0.63, 1.6]]) {   // X 브레이스
        const brace = new THREE.BoxGeometry(0.05, 0.82, 0.045);
        brace.rotateX(tilt);
        brace.rotateY(aU);
        brace.translate(ROCKET_TWR_X, ROCKET_PAD_DECK_Y + hy, ROCKET_TWR_Z);
        metalG.push(brace);
    }
    const plat = new THREE.BoxGeometry(0.44, 0.05, 0.44);
    plat.translate(ROCKET_TWR_X, ROCKET_PAD_DECK_Y + 2.32, ROCKET_TWR_Z);
    metalG.push(plat);
    const parapet = new THREE.BoxGeometry(0.03, 0.16, 0.46);   // 바깥쪽(로켓 반대편) 낮은 난간벽 — 남북으로 눕는다
    parapet.rotateY(Math.atan2(ROCKET_PAD_UX, ROCKET_PAD_UZ));
    parapet.translate(ROCKET_TWR_X + ROCKET_TWR_UX * 0.21, ROCKET_PAD_DECK_Y + 2.42, ROCKET_TWR_Z + ROCKET_TWR_UZ * 0.21);
    metalG.push(parapet);
    st.add(new THREE.Mesh(mergeGeometries(metalG, false), M(0x5a5f66)));
    st.traverse((o) => { if (o.isMesh) { o.castShadow = true; o.receiveShadow = true; } });
    stage.add(st);
    WORLD_STATIC_ROOTS.push(st);
}
...
const gantryArm = new THREE.Group();
{
    ...
    arm.castShadow = true;
    gantryArm.add(arm);
    const armRail = new THREE.Mesh(new THREE.BoxGeometry(0.025, 0.11, 0.8), M(0x5a5f66));
    armRail.position.set(0.055, 0.075, 0.45);
    gantryArm.add(armRail);
    gantryArm.rotation.y = gantryArm.userData.baseA;
    stage.add(gantryArm);
}
...
stage.add(rocketSteamPts);
const rocketDust = new THREE.Mesh(
...
const shootMat = new THREE.MeshBasicMaterial({ color: 0xeaf6ff, transparent: true, opacity: 0, blending: THREE.AdditiveBlending, depthWrite: false, fog: false });
const shootStar = new THREE.Mesh(new THREE.PlaneGeometry(2.8, 0.06), shootMat);
shootStar.visible = false;
shootStar.frustumCulled = false;
stage.add(shootStar);
...
// ---- 🌕🛰️ 우주 POI (페이즈 3): 꼬마 달 행성 + ISS풍 정거장 — 궤도(26) 위 하늘 상설. 조종간을
// 잡고 W로 올라가(한계 44) 접근 후 ⌘로 착륙/도킹, 착륙 뒤 ⌘로 하선해 표면 산책(EVA — 달은 낮은
// 중력 점프, 절친은 촐래촐래 따라다님). 지상에선 카메라 고도 게이트(y6)로 숨김 — 낮밤 아크 장식
// 달과 별개. 첫 방문은 world-space-poi(KV 동기)에 기록. 표면 높이는 poiSurfaceY(펫별 poiWalk
// 플래그로만 조회 — 전역 groundHeightAt에 섞으면 같은 x,z의 바다 물리를 오염시킨다). ----
const SPACE_POIS = [
...
SPACE_POIS[0].padPos = new THREE.Vector3(SPACE_POIS[0].x, SPACE_POIS[0].y, SPACE_POIS[0].z).addScaledVector(MOON_PAD_N, 7);
...
const poiBeaconMat = new THREE.MeshBasicMaterial({ color: 0xff7a6a, transparent: true, opacity: 0.9, fog: false });
{
    ...
    const flagN = new THREE.Vector3(-1.9, 6.55, 1.3).normalize();   // 깃발 — 산책 캡 안, 정상 곁 (걸어가서 볼 수 있다)
...
// ---- 🧑‍🚀 EVA (표면 산책): 착륙 상태 ⌘ = 하선 → 펫별 poiWalk 플래그로 표면 걷기(달=저중력 점프,
// 절친=따라다니기, 울타리 walkR) → 로켓 곁 ⌘ = 재탑승·곧장 이륙(발사대 문법 미러). Esc 해제 시
// 펫들이 알아서 로켓을 타고 귀환(빈 하늘 미아 방지 — AI 라이드로 전환). ----
function disembarkPoi() {
    const r = rocketRide, poi = ROCKET.poi;
    if (!r || !r.p) return;
    ...
    const place = (q, side) => {
        if (!q) return;
        const qx = ROCKET.x + ux * 1.5 - uz * side * 0.5;
        const qz = ROCKET.z + uz * 1.5 + ux * side * 0.5;
        q.mover.position.set(qx, poiSurfaceY(poi, qx, qz), qz);
        q.mover.rotation.set(0, Math.atan2(ux, uz), 0);
        q.swimming = false;
        q.poiWalk = poi;
        if (q !== possessed) { if (q.ai.state !== 'held') releaseAI(q); q.ai.state = 'held'; }   // 절친 — updatePoiWalk가 따라다니게 몬다
    };
    place(r.p, 1);
    place(r.friend, -1);
    rocketRide = null;
    ...
    logWorldEvent(poi.id === 'moon' ? '펫들이 달 표면에 내려섰다 — 폴짝폴짝 낮은 중력 산책 🌕👣' : '펫들이 정거장 갑판에 내려섰다 🛰️👣');
    if (poi.id === 'moon' && Math.random() < 0.6) maybeProactive(null, '달을 직접 밟았다!! 폴짝 뛰면 붕 떠오른다 — 발자국도 남을 것 같아!');
}
function boardPoiRocket() {   // 산책 → 재탑승 = 곧장 이륙 (발사대의 "타면 떠난다" 문법)
    const rider = possessed;
    const q = pets.find((o) => o !== rider && o.poiWalk);
    rider.poiWalk = null;
    rider.swimming = false;
    if (q) { q.poiWalk = null; q.swimming = false; if (q.ai.state !== 'held') { releaseAI(q); q.ai.state = 'held'; } }
    rocketRide = { p: rider, friend: q || null, t: 0, zg: 0, som: [{ cd: 6 + Math.random() * 6, t: 9 }, { cd: 10 + Math.random() * 6, t: 9 }] };
    ROCKET.padDir = -1; ROCKET.padDone = false;
    wakeSoft(4000);
    showToast('🚀 재탑승 — 이륙!');
    logWorldEvent(ROCKET.poi && ROCKET.poi.id === 'moon' ? '별똥호가 달에서 이륙했다 🌕' : '별똥호가 정거장을 떠났다 🛰️');
}
function poiAutoReturn() {   // 산책 중 빙의 해제 — 펫들이 알아서 로켓 타고 집으로
    const walkers = pets.filter((q) => q.poiWalk);
    if (!walkers.length) return;
    for (const q of walkers) { q.poiWalk = null; q.swimming = false; releaseAI(q); }
    walkers[0].ai.state = 'busy';
    if (walkers[1]) walkers[1].ai.state = 'held';
...
let aiRocketWalk = null;   // { p, ownArrive }
let rocketHop = null;      // 물가 → 갑판 승선 아크 { q, fx, fy, fz, t }
function startAiRocket(p) {
    if (rocketRide || aiRocketWalk || rocketHop || ROCKET.mode !== 'parked') return;
    if (p === possessed || p.bed || p.dip || p.pet.sleeping) return;
    let best = ISLANDS[0], bd = Infinity;   // 패드에서 가장 가까운 섬(본섬)의 마주보는 기슭
    ...
    const tx = best.x + (dx / L) * (best.r - 0.55), tz = best.z + (dz / L) * (best.r - 0.55);
    releaseAI(p);
    p.ai.state = 'goto';
    p.ai.target = { x: tx, z: tz };
    p.ai.waypoints = buildRoute(p.mover.position, { x: tx, z: tz });
    ...
    const walk = { p };
    walk.ownArrive = () => {
        if (aiRocketWalk !== walk) return;
        aiRocketWalk = null;
        if (rocketRide || rocketHop || ROCKET.mode !== 'parked' || p === possessed || p.bed || p.dip) { releaseAI(p); return; }
        p.ai.state = 'busy';   // 아크·탑승 동안 소유권 표식 (낚시/열기구 규칙)
        rocketHop = { q: p, fx: p.mover.position.x, fy: p.mover.position.y, fz: p.mover.position.z, t: 0 };
    };
    p.ai.onArrive = walk.ownArrive;
    aiRocketWalk = walk;
    logWorldEvent(`${petKo(p)}가 별똥호를 타러 발사대 기슭으로 나섰다 🚀`);
}
function updateRocketHop(delta) {
    if (!rocketHop) return;
    const h = rocketHop, q = h.q;
    if (q.bed || q.dip || q === possessed) { rocketHop = null; q.mover.position.set(h.fx, h.fy, h.fz); releaseAI(q); return; }
    h.t += delta / 1.15;
    const k = Math.min(1, h.t);
    const e = k * k * (3 - 2 * k);
    const gx = ROCKET_PAD.x - ROCKET_PAD_UX * 1.5, gz = ROCKET_PAD.z - ROCKET_PAD_UZ * 1.5;   // 핀(1.25) 밖 갑판
    q.mover.position.set(
        THREE.MathUtils.lerp(h.fx, gx, e),
        THREE.MathUtils.lerp(h.fy, ROCKET_PAD_DECK_Y, e) + Math.sin(Math.PI * k) * 1.15,
        THREE.MathUtils.lerp(h.fz, gz, e)
    );
    q.mover.rotation.y = Math.atan2(gx - h.fx, gz - h.fz);
    if (k >= 1) {
        rocketHop = null;
        q.mover.rotation.x = 0;
        q.swimming = false;
        if (rocketRide || ROCKET.mode !== 'parked') { q.mover.position.set(h.fx, h.fy, h.fz); releaseAI(q); return; }   // 그새 선점됨 — 물가로 복귀
        aiEnterRocket(q);
    }
}
function aiEnterRocket(rider) {
    releaseAI(rider);
    rider.ai.state = 'busy';   // 라이드 소유권 표식 — 디렉터가 덮어쓰면 updateRocket이 빈 여정으로 전환
    rider.swimming = false;
    let friend = null;   // 곁에 절친이 있으면 동승 (드묾 — 갑판은 배회 구역이 아님)
    const q = pets.find((o) => o !== rider);
    if (q && q !== possessed && !q.bed && !q.dip && !q.pet.sleeping
        && Math.hypot(q.mover.position.x - ROCKET_PAD.x, q.mover.position.z - ROCKET_PAD.z) < 2.3
        && (q.ai.state === 'idle' || q.ai.state === 'walk')) {
        releaseAI(q);
        q.ai.state = 'held';
        q.swimming = false;
    ...
    rocketRide = { p: rider, friend, t: 0, zg: 0, isAI: true, som: [{ cd: 6 + Math.random() * 6, t: 9 }, { cd: 10 + Math.random() * 6, t: 9 }] };
    ROCKET.mode = 'count'; ROCKET.t = 0; ROCKET.lastTick = 99; ROCKET.halfSaid = false;
    const h = currentHour(), night = h >= 18.5 || h < 6;
    logWorldEvent(`${petKo(rider)}가 별똥호 캐빈에 올랐다 — ${night ? '별을 보러 가는 야간 발사' : '혼자 우주 산책'} 카운트다운 🚀${friend ? ' (절친 동승!)' : ''}`);
    if (night) maybeProactive(null, '어두워진 하늘 아래 별똥호가 카운트다운을 시작했다 — 야간 발사 화염이 바다를 물들일 참이다!');
}
function enterRocket(rider) {
    if (!rider || rocketRide || ROCKET.mode !== 'parked') return;
    let friend = null;
    const q = pets.find((o) => o !== rider);
    if (q && !q.bed && !q.dip && !q.pet.sleeping
        && ((handHold && handHold.partner === q)
            || (Math.hypot(q.mover.position.x - ROCKET_PAD.x, q.mover.position.z - ROCKET_PAD.z) < 2.3
                && (q.ai.state === 'idle' || q.ai.state === 'walk' || q.swimming)))) {
        if (handHold) handHold = null;
        if (aiFishing && aiFishing.p === q) endAiFishing();
        releaseAI(q);
        q.ai.state = 'held';
    ...
    rocketRide = { p: rider, friend, t: 0, zg: 0, som: [{ cd: 6 + Math.random() * 6, t: 9 }, { cd: 10 + Math.random() * 6, t: 9 }] };   // zg=무중력 블렌드, som=슬로우 공중제비 타이머
    rider.swimming = false;
    running = false;
    ROCKET.mode = 'count'; ROCKET.t = 0; ROCKET.lastTick = 99; ROCKET.halfSaid = false;
    wakeSoft(8000);   // 발사 연출 순간만 60fps (토스트 3초 문법의 연장)
    playBuffer(swishBuf, { vol: 0.35, rate: 0.7, filterFreq: 800 });
    showToast(`🚀 탑승 완료 — 카운트다운 5초!${friend ? ' (절친도 함께!)' : ''} Ctrl/⌘ = 발사 취소`);
    logWorldEvent(`${petKo(rider)}가 별똥호 캐빈에 올랐다 — 발사 카운트다운${friend ? ' (절친 동승!)' : ''} 🚀`);
}
function dismountRocket() {   // 착륙/취소 하차 — 조종 펫은 갑판, AI 라이더·절친은 본섬 물가 (갑판은 배회 AI 금지 구역)
    const r = rocketRide;
    if (!r) return;
    const shoreSpot = (tang) => {   // 패드 최근접 섬 기슭 + 접선 오프셋(둘이 겹치지 않게)
        let best = ISLANDS[0], bd = Infinity;
        for (const s of ISLANDS) { const d = Math.hypot(ROCKET_PAD.x - s.x, ROCKET_PAD.z - s.z) - s.r; if (d < bd) { bd = d; best = s; } }
        const dx = ROCKET_PAD.x - best.x, dz = ROCKET_PAD.z - best.z, L = Math.hypot(dx, dz) || 1;
        const ux = dx / L, uz = dz / L;
        return { x: best.x + ux * (best.r - 0.7) - uz * tang, z: best.z + uz * (best.r - 0.7) + ux * tang };
    };
    const drop = (q, spot) => {
        q.mover.position.set(spot.x, world.groundHeightAt(spot.x, spot.z), spot.z);
        q.mover.rotation.set(0, 0, 0);
        q.swimming = false;
        if (q.ai.state === 'held' || q.ai.state === 'busy') releaseAI(q);
    };
    if (r.p) {
        if (r.isAI) drop(r.p, shoreSpot(0.55));   // AI 라이더는 물가로
        else {
            r.p.mover.position.set(ROCKET_PAD.x - ROCKET_PAD_UX * 1.35 - ROCKET_PAD_UZ * 0.65, ROCKET_PAD_DECK_Y, ROCKET_PAD.z - ROCKET_PAD_UZ * 1.35 + ROCKET_PAD_UX * 0.65);   // 핀(r≈1.25) 바깥 갑판
            r.p.mover.rotation.set(0, Math.atan2(-ROCKET_PAD.x, -ROCKET_PAD.z), 0);
            r.p.swimming = false;
            if (r.p.ai.state === 'held' || r.p.ai.state === 'busy') releaseAI(r.p);
        }
    }
...
function requestRocketExit() {   // ⌘: 카운트다운 = 취소, POI 곁 = 착륙/도킹, 착륙 완료 = 이륙, 우주 = 재진입 (Esc = 긴급)
    ...
    if (ROCKET.mode === 'moonland' || ROCKET.mode === 'dock') {
        if (ROCKET.padDir > 0 && ROCKET.padK >= 0.99) disembarkPoi();   // 착륙 완료 ⌘ = 하선 (이륙은 재탑승 ⌘) — 시퀀스 중엔 무시(연출 보호)
        return;
    }
    if (ROCKET.mode === 'manual') {
        for (const poi of SPACE_POIS) {
...
function exitRocketForce() {   // Esc/빙의 해제 — 펫은 내려주고, 비행 중이던 로켓은 빈 채로 여정을 마친다
    const r = rocketRide;
    if (!r) return;
    const preflight = ROCKET.mode === 'parked' || ROCKET.mode === 'count' || ROCKET.mode === 'ignite';
    dismountRocket();
    if (preflight) { ROCKET.mode = 'parked'; ROCKET.burn = 0; ROCKET.t = 0; }
    else rocketRide = { p: null, friend: null, t: 0, empty: true };
}
function rocketTurnHeading(target, maxStep) {
    let diff = target - ROCKET.heading;
    while (diff > Math.PI) diff -= Math.PI * 2;
    while (diff < -Math.PI) diff += Math.PI * 2;
...
function updateRocket(delta) {
    ...
    if (aiRocketWalk && (aiRocketWalk.p.ai.onArrive !== aiRocketWalk.ownArrive || aiRocketWalk.p.ai.state !== 'goto')) aiRocketWalk = null;
    // AI 라이더: 주인이 빙의하면 하이재킹(수동 전환 + 부감 글라이드), 디렉터가 뺏으면 빈 여정 완주
    if (r && r.isAI && r.p) {
    ...
    ROCKET.t += delta;
    if (ROCKET.mode === 'count') {
        const remain = 5 - ROCKET.t;
        if (r && !r.empty && !r.isAI && remain > 0 && Math.ceil(remain) !== ROCKET.lastTick) {
            ROCKET.lastTick = Math.ceil(remain);
            if (ROCKET.lastTick <= 3) { showToast(`🚀 ${ROCKET.lastTick}…`); playBuffer(swishBuf, { vol: 0.28, rate: 0.85, filterFreq: 900 }); }
        }
        if (ROCKET.t >= 5) { ROCKET.mode = 'ignite'; ROCKET.t = 0; playBuffer(swishBuf, { vol: 0.55, rate: 0.3, filterFreq: 420 }); }
    } else if (ROCKET.mode === 'ignite') {
        ROCKET.burn = Math.min(1, ROCKET.t / 0.9);
        if (ROCKET.t >= 1.15) {
            ROCKET.mode = 'ascent'; ROCKET.t = 0; ROCKET.vy = 0; ROCKET.dustT = 0;
            ROCKET.dir = Math.random() < 0.5 ? 1 : -1;   // 궤도 방향 코인플립 (열기구 문법)
            wakeSoft(9000);
            if (r && !r.empty) {
                if (!r.isAI) showToast('🚀 발사!!');
                logWorldEvent(`${r.p ? petKo(r.p) : '별똥호'}${r.friend ? `와 ${petKo(r.friend)}` : ''}가 탄 별똥호가 굉음과 함께 발사됐다 🚀`);
                maybeProactive(null, '별똥호 로켓이 발사됐다! 구름을 뚫고 우주로 솟아오르는 중!');
            }
        }
    } else if (ROCKET.mode === 'ascent') {
        ROCKET.burn = 1;
        ROCKET.vy = Math.min(4.4, ROCKET.vy + 3.2 * delta);
        ROCKET.y += ROCKET.vy * delta;
        const k = THREE.MathUtils.smoothstep(ROCKET.y, 6.5, ROCKET_APOGEE);   // 중력턴 블렌드
        ROCKET.pitch = k * 0.92;
        const entryA = ROCKET_HOME_A + ROCKET.dir * 0.55;
        ROCKET.x = THREE.MathUtils.lerp(ROCKET_PAD.x, Math.cos(entryA) * ROCKET_ORBIT_R, k);
        ROCKET.z = THREE.MathUtils.lerp(ROCKET_PAD.z, Math.sin(entryA) * ROCKET_ORBIT_R, k);
        if (k > 0.15) rocketTurnHeading(Math.atan2(-ROCKET.dir * Math.sin(entryA), ROCKET.dir * Math.cos(entryA)), delta * 1.1);
        if (ROCKET.y >= ROCKET_APOGEE) {
            ROCKET.mode = 'space'; ROCKET.t = 0;
            ROCKET.orbitA = entryA; ROCKET.lapAcc = 0;
            ROCKET.camGlide = 1.8;   // 탑승 카메라를 부감 명당으로 스르륵 — 발밑 군도가 프레임에 들어온다
            wakeSoft(6000);
            if (r && !r.empty) {
        ...
        ROCKET.burn = 0.18 + Math.max(0, Math.sin(ROCKET.t * 2.1)) * 0.1;   // RCS 잔불
        const dA = ROCKET.dir * ((Math.PI * 2) / 58) * delta;
        ROCKET.orbitA += dA;
        ROCKET.lapAcc += Math.abs(dA);
        ROCKET.x = Math.cos(ROCKET.orbitA) * ROCKET_ORBIT_R;
        ROCKET.z = Math.sin(ROCKET.orbitA) * ROCKET_ORBIT_R;
        ROCKET.y = ROCKET_APOGEE + Math.sin(ROCKET.t * 0.5) * 0.16;
        ROCKET.pitch = THREE.MathUtils.lerp(ROCKET.pitch, 0.9, Math.min(1, delta * 1.4));   // 순항 자세 — 돔은 위를 향한 채
        rocketTurnHeading(Math.atan2(-ROCKET.dir * Math.sin(ROCKET.orbitA), ROCKET.dir * Math.cos(ROCKET.orbitA)), delta * 1.6);
        if (r && !r.empty && !ROCKET.halfSaid && ROCKET.lapAcc > Math.PI) {
            ROCKET.halfSaid = true;
            if (Math.random() < 0.6) maybeProactive(null, '궤도 반 바퀴 — 무인도 링과 바다 전체가 한눈에 내려다보인다!');
        }
        if (ROCKET.lapAcc >= Math.PI * 2) {
            ROCKET.mode = 'descent'; ROCKET.t = 0;
        ...
        for (const poi of SPACE_POIS) {   // POI 몸체 회피 — 달 구·정거장 코어를 뚫지 않게 표면으로 밀어낸다
            const cy = poi.R ? poi.y : poi.y + 3.8;
            const cz = poi.R ? poi.z : poi.z - 2.9;
            const dx = ROCKET.x - poi.x, dy = ROCKET.y - cy, dz = ROCKET.z - cz;
            const d3 = Math.hypot(dx, dy, dz);
            if (d3 < poi.avoid && d3 > 0.001) {
                const k = poi.avoid / d3;
                ROCKET.x = poi.x + dx * k;
                ROCKET.y = cy + dy * k;
                ROCKET.z = cz + dz * k;
                ROCKET.spd *= 0.4;
        ...
        const d = Math.hypot(dx, dz);
        if (d > 0.25) {
            const step = Math.min(d, 2.7 * delta);
            ROCKET.x += (dx / d) * step;
            ROCKET.z += (dz / d) * step;
            rocketTurnHeading(Math.atan2(dx, dz), delta * 1.5);
        }
        ROCKET.y = Math.max(ROCKET.y - (1.5 + Math.min(1.4, d * 0.1)) * delta, 5.2);
        ROCKET.pitch = THREE.MathUtils.lerp(ROCKET.pitch, d > 1.5 ? 0.55 : 0, Math.min(1, delta * 1.8));
        if (d <= 0.3 && ROCKET.y <= 5.25) { ROCKET.mode = 'retro'; ROCKET.t = 0; ROCKET.vy = -1.7; }
    } else if (ROCKET.mode === 'retro') {
        ...
        if (ROCKET.y <= ROCKET_PAD_DECK_Y) {
            ...
            if (r) {
                if (!r.empty) {
                    if (!r.isAI) showToast('🚀 무사 귀환! 별똥호 우주 여행 완료');
                    logWorldEvent('별똥호가 역추진 착륙으로 발사대에 사뿐히 내려앉았다 🚀');
                    maybeProactive(null, '우주 여행을 마치고 무사히 착륙했다! 정말 굉장한 모험이었다!');
                }
                dismountRocket();
            }
        }
    ...
    ud.flamePts.visible = flameK > 0.02;
    ud.core.visible = ROCKET.burn > 0.02;
    ud.outer.visible = flameK > 0.02;
    if (inSpace) {
        ud.coreMat.color.setHex(0xcfe4ff);
        ud.coreMat.opacity = 0.2 + Math.max(0, Math.sin(ROCKET.t * 2.1)) * 0.3;
        ud.core.scale.set(0.5, 0.35, 0.5);
    } else {
        ud.coreMat.color.setHex(0xfff3cd);
        ud.coreMat.opacity = ROCKET.burn * 0.9 * flick;
        ud.core.scale.set(1, (0.7 + ROCKET.burn * 0.5) * flick, 1);
        ud.outerMat.opacity = ROCKET.burn * 0.55 * flick;
        ud.outer.scale.set(0.9 + ROCKET.burn * 0.25, (0.6 + ROCKET.burn * 0.6) * flick, 0.9 + ROCKET.burn * 0.25);
    }
    // 연기 트레일 — 발사·역추진의 8할. 재진입(descent)·우주에선 없음
    const smokeTarget = !inSpace && ROCKET.mode !== 'descent' ? ROCKET.burn * 0.8 : 0;
    ud.smokeMat.opacity += (smokeTarget - ud.smokeMat.opacity) * Math.min(1, delta * (smokeTarget > ud.smokeMat.opacity ? 5 : 1.4));
    ud.smokePts.visible = ud.smokeMat.opacity > 0.02;
    ud.beaconMat.opacity = 0.3 + 0.7 * Math.abs(Math.sin(wxTime.value * (ROCKET.mode === 'count' ? 9 : 2.2)));
    const hNow = currentHour();
    const wantLight = ROCKET.burn > 0.05 && !inSpace && (hNow >= 18.5 || hNow < 6);
    if (wantLight !== rocketLightOn) { rocketLightOn = wantLight; if (wantLight) scene.add(rocketBurnLight); else scene.remove(rocketBurnLight); }
    if (rocketLightOn) {
    ...
    rocketSteamPts.visible = sm.opacity > 0.02;
    if (ROCKET.dustT < 0.9) {   // 이륙·착지 먼지 링 — 확장 토러스 1드로우
        ROCKET.dustT += delta;
        const k = Math.min(1, ROCKET.dustT / 0.85);
        rocketDust.visible = k < 1;
        rocketDust.scale.setScalar(0.55 + k * 2.6);
        rocketDust.material.opacity = (1 - k) * 0.85;
    } else rocketDust.visible = false;
    const armTarget = ROCKET.mode === 'parked' && !rocketRide ? 0 : 1;   // 정비 암: 대기 = 로켓에 밀착, 발사 준비 = 젖힘
    ROCKET.armK += THREE.MathUtils.clamp(armTarget - ROCKET.armK, -delta * 1.1, delta * 1.1);
    gantryArm.rotation.y = gantryArm.userData.baseA + ROCKET.armK * 2.1;
    // 우주 진입 카메라 글라이드 (1회성): "위+궤도 바깥" 명당으로 스르륵 → 이후엔 유저 오빗 자유
    if (ROCKET.camGlide > 0) {
        if (r && !r.empty && (r.p === possessed || r.friend === possessed)) {
            const uL = Math.hypot(ROCKET.x, ROCKET.z) || 1;
            _rocketCam.set(ROCKET.x + (ROCKET.x / uL) * 3.2, ROCKET.y + 4.8, ROCKET.z + (ROCKET.z / uL) * 3.2);   // ~34° 부감 — 로켓 너머 발밑 군도 미니어처 (어두운 우주는 마우스로 올려보는 몫)
            camera.position.lerp(_rocketCam, Math.min(1, delta * 2.4));
        }
        ROCKET.camGlide = Math.max(0, ROCKET.camGlide - delta);
    }
    // 🌠 별똥별: 우주 순항 중 7~14초마다 한 줄기 — 카메라 기준 상공에서 긋고 사라진다
    if (spaceF > 0.85 && shootLife >= 1) {
        shootCd -= delta;
        if (shootCd <= 0) {
            shootCd = 7 + Math.random() * 7;
            shootLife = 0;
            const az = Math.random() * Math.PI * 2, el = 0.45 + Math.random() * 0.5;
            shootStar.position.set(
                camera.position.x + Math.cos(az) * Math.cos(el) * 26,
                camera.position.y + Math.sin(el) * 26,
                camera.position.z + Math.sin(az) * Math.cos(el) * 26
            );
            _shootVel.set(Math.cos(az + 2.2), -0.25 - Math.random() * 0.35, Math.sin(az + 2.2)).normalize().multiplyScalar(15);
        }
    }
    if (shootLife < 1) {
        shootLife = Math.min(1, shootLife + delta * 1.1);
        shootStar.position.addScaledVector(_shootVel, delta);
        _shootX.copy(_shootVel).normalize();                                        // 스트릭 장축 = 진행 방향
        _shootZ.subVectors(camera.position, shootStar.position).normalize();        // 면은 카메라를 향해
        _shootY.crossVectors(_shootZ, _shootX).normalize();
        _shootZ.crossVectors(_shootX, _shootY);
        shootStar.quaternion.setFromRotationMatrix(_shootM.makeBasis(_shootX, _shootY, _shootZ));
    ...
    for (const q of pets) {
        ...
        const poi = q.poiWalk;
        const mp_ = q.mover.position;
        const dx = mp_.x - poi.x, dz = mp_.z - poi.z;
        const d = Math.hypot(dx, dz);
        ...
        mp_.y = poiSurfaceY(poi, mp_.x, mp_.z);
        q.mover.rotation.x = 0;
        q.mover.rotation.z = 0;
    }
}
...
function updateRocketPose(delta) {   // 엔티티 업데이트 뒤 덮어쓰기 (비행기 문법) — 앉아 창밖 구경, 우주에선 좌석을 떠나 무중력 유영
    ...
    const pose = (q, fwd, ph, idx) => {
        ...
        let ry = ROCKET.heading + (fwd > 0 ? 0 : Math.PI);
        if (zg > 0.001) {
            // 무중력 유영: 리사주 드리프트(위상차 — 둘이 절대 안 겹침) + 기우뚱 텀블 + 이따금 슬로우 공중제비
            _rocketZgV.set(
                Math.sin(t * 0.53 + ph) * 0.045 + Math.cos(ph) * 0.028,
                1.0 + Math.sin(t * 0.77 + ph * 1.3) * 0.026,
                Math.cos(t * 0.41 + ph) * 0.045 + Math.sin(ph) * 0.028
            ).multiplyScalar(RK_SCALE).applyQuaternion(rocketGroup.quaternion).add(rocketGroup.position);
            _rocketSeat.lerp(_rocketZgV, zg);
            const so = r.som[idx];
            if (zg > 0.9 && so.t >= 3) { so.cd -= delta; if (so.cd <= 0) { so.t = 0; so.cd = 8 + Math.random() * 8; } }
            if (so.t < 3) { so.t += delta; rx += Math.PI * 2 * THREE.MathUtils.smoothstep(so.t / 3, 0, 1) * zg; }
            rx += (Math.sin(t * 0.33 + ph) * 0.3 - ROCKET.pitch * 0.4) * zg;
            ry += Math.sin(t * 0.21 + ph) * 0.6 * zg;   // 느리게 두리번 — 창밖 구경
            rz += Math.sin(t * 0.27 + ph * 2) * 0.22 * zg;
        }
        q.mover.position.set(_rocketSeat.x, _rocketSeat.y, _rocketSeat.z);
        q.mover.rotation.set(rx, ry, rz);
        q.pet.walking = false;
        q.swimming = false;
        for (const f of q.pet.feet) f.rotation.x = -1.35 * (1 - zg) - 0.35 * zg;   // 앉기 다리접기(벤치 문법) ↔ 유영 다리 축 늘어짐
        for (const er of q.pet.ears) er.rotation.x = (er.userData._restRotX || 0) + 0.55 * wind - 0.28 * zg;   // 상승 맞바람 / 무중력엔 귀가 둥실
    };
    pose(r.p, 1, 0, 0);
    pose(r.friend, -1, 2.4, 1);
}
...
function makeFerryRoute() {
    ...
    for (let attempt = 0; attempt < 25; attempt++) {
        ...
        for (let i = 0; i < 160 && ok; i++) {
            ...
            if (Math.hypot(pt.x - ROCKET_PAD.x, pt.z - ROCKET_PAD.z) < ROCKET_PAD_R + 0.6) ok = false;   // 🚀 발사 플랫폼
        }
        if (!ok) continue;
...
let dive = null;   // { p, t, phase: down|under(AI 블롭)|deep(빙의 실잠수)|up, blob, bubbleAt, force }
// ---- 🌊 B단계 해저: 카메라가 수면 아래로 들어가면 안개·하늘·수면 밑면이 물속 세계로 전환.
// 해저 지형 seabedHeight: 근해 셸프(-1.1)→외해 사구(-4~-6)→EXPLORE_R 밖 심연 절벽.
// 렌더 규율: 수중 안개(near 2/far 16)가 원거리 컬링을 겸한다 — 포스트 이펙트 0. ----
function seabedHeight(x, z) {
    let base = -4.2 - 1.1 * Math.sin(x * 0.21 + 1.7) * Math.sin(z * 0.19 - 0.6) - 0.7 * Math.sin((x + z) * 0.11 + 0.8);
    let shelf = 1e9;
    for (const isl of ISLANDS) shelf = Math.min(shelf, Math.hypot(x - isl.x, z - isl.z) - isl.r);
    if (shelf < 6) base = THREE.MathUtils.lerp(-1.1, base, THREE.MathUtils.smoothstep(shelf, 0, 6));   // 근해 셸프
    const d = Math.hypot(x, z);
    if (d > EXPLORE_R - 2) base -= (d - (EXPLORE_R - 2)) * 6;   // 심연 절벽 — 리시가 낭떠러지 서사를 얻는다
    return base;
}
const UNDERWATER_FOG = new THREE.Color(0x123a46);
let underPlane = null;   // 수면 밑면 — 아래서 올려다보는 밝은 '천장'
function ensureUnderPlane() {
    if (underPlane) return;
    underPlane = new THREE.Mesh(
        new THREE.CircleGeometry(84, 40),
        new THREE.MeshBasicMaterial({ color: 0x9adcee, transparent: true, opacity: 0.5, side: THREE.DoubleSide, depthWrite: false, fog: true, toneMapped: false })   // fog:true — 천장이 원거리에서 물색으로 잦아들어야 far 클립 경계가 안 보인다
    );
    underPlane.rotation.x = Math.PI / 2;
    underPlane.visible = false;
...
function updateUnderwater() {
    ...
    camUnderwater = now;
    skyDomeMesh.visible = !now;
    sunMesh.visible = !now;
    underPlane.visible = now;
    if (now) {   // 물속 — 안개가 분위기이자 컬링 (background도 물색 — far 너머 빈 공간이 하늘색이면 깨진다)
        scene.fog.color.copy(UNDERWATER_FOG);
        scene.fog.near = 2;
        scene.fog.far = 16;
        scene.background.copy(UNDERWATER_FOG);
        camera.far = 18;   // 안개 너머는 어차피 안 보인다 — far를 당겨야 지상 드로우가 진짜로 컬링된다 (근해 실측 324→베이스 회귀)
        camera.updateProjectionMatrix();
    } else {   // 물 밖 — 해저 전부 내리고 안개·하늘·far 즉시 원복 (스로틀 대기 시 최대 30초 물안개가 남는다)
        clearSeaFloor();
        camera.far = 100;   // 생성 시 기본값
        camera.updateProjectionMatrix();
        updateDayNight(true);
    }
}
...
function endDive(caught) {
    ...
    if (!isAI) clearSeaFloor();
    p.pet.wrap.visible = true;
    if (blob) { scene.remove(blob); blob.material.dispose(); }
    dive = null;
...
function updateDive(delta) {
    ...
    dive.t += delta;
    const m = p.mover;
    if (dive.blob) dive.blob.position.set(m.position.x, waveYAt(m.position.x, m.position.z) + 0.015, m.position.z);   // 빙의 딥 다이브는 블롭 없음 (B단계)
    if (dive.phase === 'down') {
        ...
        if (dive.t >= 0.45) {
            ...
            if (dive.isAI) {
                dive.phase = 'under';
                p.pet.wrap.visible = false;   // AI: 실루엣 블롭이 연기한다 (A단계)
            } else {
                dive.phase = 'deep';           // 빙의: 진짜 물속으로 (B단계)
                m.position.y = waveYAt(m.position.x, m.position.z) - 0.9;
            }
        }
        ...
        dive.bubbleAt -= delta;
        if (dive.bubbleAt <= 0) {   // 🫧 입에서 새는 기포
            dive.bubbleAt = 0.55 + Math.random() * 0.4;
            const bub = new THREE.Sprite(new THREE.SpriteMaterial({ map: blobTex, color: 0xd8f2ff, transparent: true, opacity: 0.7, depthWrite: false }));
            bub.scale.setScalar(0.03 + Math.random() * 0.035);
            bub.position.set(m.position.x, m.position.y + 0.12, m.position.z);
            scene.add(bub);
        ...
        m.position.y = Math.min(ceil - 0.1, m.position.y + delta * 2.6);
        m.rotation.x = -0.3;   // 코 들고 부상 (수면 복귀 후 lean은 updatePlayer가 되쓴다)
        p.pet.wrap.rotation.x = 0;
        if (m.position.y >= ceil - 0.12) endDive();   // 수면 도착 — 평소 수영으로
...
// ---- 🌊 해저 바닥: 수중(딥 다이브)에서만 사는 청크 스트리밍 — 무인도 문법(프레임당 1작업·
// dispose는 지오메트리만). 모래 사구 메시 + 시드 드레싱(바위·켈프·산호는 셸프 수심별) +
// 해저 보물 ✕(72h 재보급) + 난파선 1척(고정 시드 좌표) + 물고기 떼 인스턴싱. 수면 위 비용 0. ----
const SEA_CHUNK = 24;
const seaChunks = new Map();
const seaQueue = [];
const seaQueued = new Set();
let seaSpawns = [];      // 바닥에 보이는 채집물 { t, x, z, mesh }
let seaSpawnAt = 0;
let dugSeabed = {};
...
function loadSeaChunk(key, cx, cz) {
    ...
    const info = seaChunkInfo(cx, cz);
    const rng = seededRand(info.seed + 733);
    const group = new THREE.Group();
    // 바닥: 사구 변위 그리드
    const geo = new THREE.PlaneGeometry(SEA_CHUNK, SEA_CHUNK, 10, 10);
    geo.rotateX(-Math.PI / 2);
    const pos = geo.attributes.position;
    for (let i = 0; i < pos.count; i++) {
        const wx = pos.getX(i) + (cx + 0.5) * SEA_CHUNK, wz = pos.getZ(i) + (cz + 0.5) * SEA_CHUNK;
        pos.setY(i, seabedHeight(wx, wz));
    }
    geo.computeVertexNormals();
    const floor = new THREE.Mesh(geo, seaSandMat);
    ...
    const spot = () => {
        ...
        return { x, z, y: seabedHeight(x, z) };
    };
    for (let i = 0; i < info.rocks; i++) {
        const sp = spot();
        if (!sp) continue;
        const rock = new THREE.IcosahedronGeometry(0.16 + rng() * 0.22, 0);
        rock.scale(1, 0.7, 1);
        rock.rotateY(rng() * Math.PI);
    ...
    if (shelfHere < 3.2) for (let i = 0; i < info.coral; i++) {   // 산호는 얕은 셸프에서만
        const sp = spot();
        if (!sp) continue;
        const col = rng() < 0.5 ? [0xff8a7a, 0xd0503f] : [0xffb46a, 0xd9803a];
        for (let br = 0; br < 4; br++) {   // 가지 산호
            const branch = new THREE.CylinderGeometry(0.02, 0.035, 0.24 + rng() * 0.2, 6);
            branch.rotateZ((rng() - 0.5) * 1.1);
            branch.rotateY(rng() * Math.PI * 2);
            branch.translate(sp.x + (rng() - 0.5) * 0.18, sp.y + 0.14, sp.z + (rng() - 0.5) * 0.18);
            grad.push(bakeGrad(branch, col[0], col[1], { curve: 1.2 }));
        }
        const fan = new THREE.SphereGeometry(0.16, 8, 5, 0, Math.PI);   // 부채 산호
        fan.scale(1, 0.9, 0.12);
    ...
    if (info.treasure && !seabedDug(key) && islandOf(info.tx, info.tz) < 0) {
        ...
        const mound = new THREE.Mesh(new THREE.SphereGeometry(0.18, 9, 6), M(0xd9c898));
        mound.scale.y = 0.3;
        mound.position.set(info.tx, ty + 0.02, info.tz);
        treasure.add(mound);
        for (const rot of [0.8, -0.8]) {
            const plank = new THREE.Mesh(new THREE.BoxGeometry(0.44, 0.018, 0.08), M(0x5c4630));
            plank.rotation.y = rot;
            plank.position.set(info.tx, ty + 0.06, info.tz);
            treasure.add(plank);
        }
        group.add(treasure);
    }
    // 🚢 난파선 — 이 청크에 고정 좌표가 들어오면
    if (WRECK_AT.x >= cx * SEA_CHUNK && WRECK_AT.x < (cx + 1) * SEA_CHUNK && WRECK_AT.z >= cz * SEA_CHUNK && WRECK_AT.z < (cz + 1) * SEA_CHUNK) {
        const wy = seabedHeight(WRECK_AT.x, WRECK_AT.z);
        const hullGeo = new THREE.CylinderGeometry(0.65, 0.85, 2.6, 10, 1, false, 0, Math.PI);
        hullGeo.rotateZ(Math.PI / 2);
        hullGeo.rotateX(-0.28);
        hullGeo.translate(WRECK_AT.x, wy + 0.32, WRECK_AT.z);
        const hull = new THREE.Mesh(bakeGrad(hullGeo, 0x6a5138, 0x3c2c1c, { curve: 1 }), gradMat);
        group.add(hull);
        const mastGeo = new THREE.CylinderGeometry(0.05, 0.07, 1.9, 7);
        mastGeo.rotateZ(0.9);
        mastGeo.translate(WRECK_AT.x - 0.5, wy + 0.9, WRECK_AT.z + 0.3);
        group.add(new THREE.Mesh(bakeGrad(mastGeo, 0x5c4630, 0x35281a, { curve: 1 }), gradMat));
        const chest = new THREE.Mesh(new THREE.BoxGeometry(0.3, 0.2, 0.22), M(0x8a5a30));
        chest.position.set(WRECK_AT.x + 0.9, wy + 0.1, WRECK_AT.z - 0.5);
...
function clearSeaFloor() {   // 부상/수면 복귀 — 전부 내리고 채집물도 정리
    for (const key of [...seaChunks.keys()]) unloadSeaChunk(key);
    seaQueue.length = 0;
    seaQueued.clear();
    for (const sp of seaSpawns) { sp.mesh.traverse((o) => { if (o.isMesh) o.geometry.dispose(); }); fruitLayer.remove(sp.mesh); }
    seaSpawns = [];
    if (seaFishMesh) seaFishMesh.visible = false;
}
let seaTickT = 0;
function updateSeaFloor(delta) {
    seaTickT += delta;
    if (seaTickT >= 0.8) {   // 청크 스캔 — 카메라 주변
        seaTickT = 0;
        const tx = camera.position.x, tz = camera.position.z;
        const want = new Set();
        for (let cx = Math.floor((tx - 26) / SEA_CHUNK); cx <= Math.floor((tx + 26) / SEA_CHUNK); cx++) {
            for (let cz = Math.floor((tz - 26) / SEA_CHUNK); cz <= Math.floor((tz + 26) / SEA_CHUNK); cz++) {
                const key = `${cx},${cz}`;
                want.add(key);
                if (!seaChunks.has(key) && !seaQueued.has(key)) { seaQueued.add(key); seaQueue.push({ op: 'load', key, cx, cz }); }
            }
        }
    ...
    if (seaSpawnAt <= 0) {
        seaSpawnAt = 2.6;
        seaSpawns = seaSpawns.filter((sp) => {
            if (Math.hypot(sp.x - p.mover.position.x, sp.z - p.mover.position.z) < 15) return true;
            sp.mesh.traverse((o) => { if (o.isMesh) o.geometry.dispose(); });
            fruitLayer.remove(sp.mesh);
            return false;
        });
        if (seaSpawns.length < 4) {
    ...
    updateSeaFish(delta, p);
}
// 🐟 물고기 떼 — InstancedMesh 하나(10마리), 표류 중심 주위 원형 유영
...
function collectNearbySeafood() {   // deep 중 ⌘ — 바닥의 실물 수거
    const p = dive && dive.p;
    if (!p) return false;
    let best = null, bd = 0.95;
    for (const sp of seaSpawns) {
        const d = Math.hypot(sp.x - p.mover.position.x, sp.z - p.mover.position.z);
        if (d < bd) { bd = d; best = sp; }
    }
    if (!best) return false;
    seaSpawns.splice(seaSpawns.indexOf(best), 1);
    best.mesh.traverse((o) => { if (o.isMesh) o.geometry.dispose(); });
    fruitLayer.remove(best.mesh);
    recordSeafoodCatch(p, best.t);
    return true;
}
function digNearbySeaTreasure() {   // deep 중 ⌘ — ✕ 발굴 (무인도 보상 테이블)
    const p = dive && dive.p;
    if (!p) return false;
    for (const [key, e] of seaChunks) {
        ...
        e.treasure = null;
        triggerHugBurst(p.mover.position.x, p.mover.position.y + 0.2, p.mover.position.z);
        fishFanfare();
        ...
        if (roll < 0.5) {
            ...
            showToast(`🤿🎁 해저 보물 — ${label} 코디가 열렸어요!`);
            logWorldEvent(`해저 보물 상자에서 ${label} 코디를 찾았다 🤿🎁`);
        } else {
            showToast('🤿✨ 해저 보물 — 반짝이는 옛 동전이다');
            logWorldEvent('해저 보물에서 반짝이는 옛 동전을 찾았다 🤿✨');
        }
        return true;
    }
    return false;
}
function recordSeafoodCatch(p, caught) {   // 도감 기록 + 연출 (수거·AI 공용 아님 — 주인 전용)
    const counts = seafoodCounts();
    const first = !counts[caught.id];
    counts[caught.id] = (counts[caught.id] || 0) + 1;
    try { localStorage.setItem('world-seafood', JSON.stringify(counts)); worldSync('world-seafood'); } catch (e) {}
...
function animate() {
    ...
    updateRocketPose(delta);                 // 🚀 캐빈 좌석·무중력 유영 — 등맞대기/리사주 부유 (엔티티 뒤 덮어쓰기)
    ...
    syncDiveBtn();                           // 🤿/⬆️ 문맥 버튼
    updateDive(delta);                       // 🤿 잠수 — AI 블롭 / 빙의 딥 다이브 (엔티티 뒤)
...
// TEMP DRINK LAB — ?drinklab: 음료 9종 × 마시기 0/1/2 격자 (검수용, 커밋 전 제거)
if (location.search.includes('drinklab')) {
    const _dl = new THREE.DirectionalLight(0xffffff, 1.6); _dl.position.set(4, 26, 12); scene.add(_dl);
    ...
    DRINKS.forEach((dk, i) => {
        for (let sp = 0; sp <= 2; sp++) {
            try {
                ...
                scene.add(grp);
            } catch (e) { console.error('drinklab', dk.id, sp, e); }
        }
    });
...
if (location.search.includes('foodlab')) {
    const _fl = new THREE.DirectionalLight(0xffffff, 1.6); _fl.position.set(4, 26, 12); scene.add(_fl);
    scene.add(new THREE.AmbientLight(0xffffff, 0.5));
    FOODS.forEach((f, i) => {
        for (let bt = 0; bt <= 2; bt++) {
            try {
                const m = new THREE.Mesh(makeFoodGeo(f, bt), gradMat);
                m.scale.setScalar(9);
