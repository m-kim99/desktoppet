...
export const ISLANDS = [
    ...
    // 농장 섬 — 텃밭 확장 이전(8칸)·허수아비·퇴비함. 간격은 표준(2.6)보다 한 발 물러난 본섬 3.6m
    // (서쪽 뷰 숨통 — 모래섬 3.4 선례), SW 추억의 섬과 2.8m. 다리 = BRIDGES[4].
    { x: -12.2,  z: 3.9,   r: 3.0 },
];
...
export const BRIDGES = [
    ...
    null,   // [3] 휴양지 모래섬 — 다리 없음(보트/수영 전용)의 자리 지킴이 홀. 순회 코드는 if (!br) continue 가드 필수
    { A: { x: -5.43, z: 1.74 }, B: { x: -9.82, z: 3.14 }, inner: { x: -5.05, z: 1.62 }, outer: { x: -10.20, z: 3.26 } },   // 농장 섬 다리 — span 4.6m (플랭크 밀도는 길이 기준이라 그대로 촘촘)
];
...
export const FERRY_SEA_POINTS = [   // 외해 링 (시계방향) — 전부 열린 물, 수평선 경계 19.6 안.
    ...
    { x: -0.6, z: 16.6 }, { x: -4.5, z: 16 }, { x: -11, z: 12 }, { x: -18.3, z: 3.5 }, { x: -14.5, z: -11 },   // 서쪽 부표는 농장 섬 신설로 -16,2 → -18.3,3.5 (섬 여유 4.4m, 수평선 18.6 < 19.6)
...
export const FLAT_SPOTS = [
    ...
    { x: -12.95, z: 4.25, r: 1.8, follow: 'garden-1' },   // 농장 섬 밭 받침 — 2.78m 프레임이 구릉에 기울지 않게 (그네 패드 원리)
];
...
export const PROPS = [
    ...
    { type: 'fence', x: -5.25, z:  0.1,  rotY: 1.35, r: 0.5 },    // W 림 울타리 조각 — 농장 다리 상륙지에서 남쪽으로 비켜남 (MOVED_DEFAULTS 동반)
    ...
    // 🚜 농장 섬으로 이전 + 8칸 확장(2×4) — 본섬 옛 자리는 림까지 1.0m라 확장 불가였다(실측).
    // ISLAND_SIG 변경이 위성섬 프롭 저장 좌표를 자동 폐기하므로 사용자 저장본도 새 기본으로 스냅.
    { type: 'garden',     x: -12.95, z: 4.25,  rotY: 1.87,   r: 1.4 },
    { type: 'scarecrow',  x: -10.6,  z: 5.3,   rotY: -1.99,  r: 0.22 },   // 밭을 바라보는 허수아비
    { type: 'compost',    x: -11.5,  z: 2.0,   rotY: 0.6,    r: 0.3 },    // 퇴비함 — 남동 림
