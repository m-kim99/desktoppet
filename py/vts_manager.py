...
class VTSManager:
    def __init__(self):
        ...
        # Match the actual high volume of the TTS
    ...
    async def vts_worker(self):
        ...
        while self.is_running:
            try:
                ...
                # ====== Core fix area: handle an odd number of bytes ======
                if len(pcm_data) % 2 != 0:
                    pcm_data = pcm_data[:-1] # If odd, drop the last byte
                
                if not pcm_data: # If empty after dropping, skip
                ...
                for i in range(0, len(samples), samples_per_frame):
                    ...
                    if debug_counter % 10 == 0 and rms >= 400:
                        pass
                        # logger.info(f"[LipSync Debug] RMS: {rms:.1f} | scaled volume: {volume_factor:.2f} | vowel ratio: {vowel_ratio:.2f} | final -> mouth open: {self.mouth_value:.2f}")
