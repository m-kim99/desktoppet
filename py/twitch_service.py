    ...
    def _handle_line(self, line: str):
        ...
        if line.startswith("@"):
            ...
            for kv in tag_str.split(";"):
                if "=" in kv:
                    k, v = kv.split("=", 1)
                    # Handle escape characters in the Twitch IRC protocol
                    v = v.replace("\\s", " ").replace("\\:", ";").replace("\\r", "\r").replace("\\n", "\n")
                    tags[k] = v

        # 2. Identify the IRC command (PRIVMSG or USERNOTICE)
        parts = line.split(" ")
        command = ""
        for p in parts:
        ...
        if command == "PRIVMSG":
            ...
            danmu_type = "danmaku"
            try:
                # Take everything after ':' as the message body
                msg_content = line.split(" :", maxsplit=1)[1]
            except: 
                msg_content = ""

        elif command == "USERNOTICE":
            # --- Complex system events (subscriptions, gifts, raids, etc.) ---
            msg_id = tags.get("msg-id", "")
            system_msg = tags.get("system-msg", "") # The official English description generated by Twitch
            
            # Try extracting the custom text users attach to subs/announcements
            user_text = ""
            try:
            ...
            if msg_id in ["sub", "resub"]:
                # Subscription or resub -> equivalent to a paid-membership event
                ...
                # Gifted subscription -> equivalent to a gift event
                danmu_type = "gift"
                msg_content = system_msg
                
            elif msg_id == "raid":
                # Raid (another streamer brings viewers in) -> equivalent to a room-enter event
                danmu_type = "enter_room"
                msg_content = system_msg
                
            elif msg_id == "announcement":
                # Channel announcement -> treated as a regular message, but with a label prefixed
                danmu_type = "danmaku"
                msg_content = f"[Announcement] {user_text}"
                
            else:
                # Handling for other system events
                danmu_type = "danmaku"
                msg_content = system_msg if system_msg else user_text

        # 5. Call back to the service layer
        if self._callback:
            # This callback must accept four arguments: channel, user, msg_content, danmu_type
            asyncio.create_task(
