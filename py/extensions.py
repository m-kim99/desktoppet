...
def parse_repo_urls(repo_url: str) -> tuple[Optional[str], list[str]]:
    ...
    if 'github.com' in parsed.netloc.lower() and len(path_parts) >= 2:
        ...
        # Default to generating a JihuLab (GitLab) backup direct link, covering both common main branches: main and master [1]
        jihulab_urls = [
...
# In-memory task-state storage (Redis recommended in production)
...
async def download_zip(url: str, dest: Path, timeout: float = 60.0) -> None:
    ...
    # Validate the ZIP archive header
...
def _do_zip_install(zip_url: str, temp_dir: Path, target: Path, ext_id: str) -> None:
    ...
    # Delete the old directory directly
...
def _run_bg_install(repo_url: str, ext_id: str, backup_url: str = ""):
    ...
    try:
        ...
        # Force the JihuLab (GitLab) same-name mirror as default, replacing third-party backup URLs [1]
        github_url, jihulab_urls = parse_repo_urls(repo_url)
        
        urls = []
        github_connected = False
        
        # Briefly test GitHub connectivity for 3 seconds [1]
        try:
            import httpx
        ...
        # Dynamically order download sources based on connectivity [1]
        if github_connected:
            # When reachable: GitHub first, JihuLab (GitLab) same-name backup as fallback [1]
            if github_url:
                urls.append(github_url)
            urls.extend(jihulab_urls)
        else:
            # When blocked: JihuLab (GitLab) first, direct GitHub tried last [1]
            urls.extend(jihulab_urls)
            if github_url:
                urls.append(github_url)
        ...
        for i, zip_url in enumerate(urls):
            ...
            try:
                ...
                # Check whether npm install is needed
...
async def install_from_github(req: GitHubInstallRequest, background: BackgroundTasks):
    ...
    # Check whether a task is already in progress
    ...
    # Uniformly ignore the incoming backupUrl, letting parse_repo_urls do the unified JihuLab (GitLab) closed-loop mapping [1]
...
async def get_task_status(ext_id: str):
    ...
    if not status:
        # Check whether installation has completed
        target = Path(EXT_DIR) / ext_id
        if target.exists():
...
def update_extension(ext_id: str):
    ...
    # Force JihuLab (GitLab) as the default GitHub mirror fallback, ignoring the external backupRepository field [1]
    github_url, jihulab_urls = parse_repo_urls(main_repo)
    
    urls = []
    github_connected = False
    
    # Test GitHub connectivity for 3 seconds [1]
    try:
        with httpx.Client(timeout=3) as c:
            c.head("https://github.com")
        github_connected = True
