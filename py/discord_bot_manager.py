...
class DiscordClient(discord.Client):
    def __init__(self, config: DiscordBotConfig, manager: DiscordBotManager):
        ...
        # Core: track the running task for each channel
    ...
    async def on_message(self, msg: discord.Message):
        ...
        # Non-empty = only process messages from listed user IDs, silently ignore the rest.
        allowed = getattr(self.config, "allowedUserIds", None) or []
        if allowed and str(msg.author.id) not in {str(x).strip() for x in allowed if str(x).strip()}:
            logging.warning(f"Discord: 已拦截未授权用户的消息: {msg.author.id}")
            return

        cid = msg.channel.id
        
        # 1. Shortcut commands and task-interruption check
        ...
        # 2. If there's a task currently running, interrupt it
        if cid in self.active_tasks:
