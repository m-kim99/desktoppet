...
class TaskCreateRequest(BaseModel):
    ...
    run_at_time: Optional[str] = None  # Scheduled-task timestamp (ISO format)
    interval_minutes: int = 60         # Interval for cyclic tasks
    platforms: List[str] = []    # New: multi-channel support
    trigger_config: Optional[dict] = {}

# 1. Add a task-type enum
class TaskType(str, Enum):
    ONCE = "once"           # One-off task (runs immediately)
    SCHEDULED = "scheduled" # Scheduled task (runs once at a future time)
    RECURRING = "recurring" # Cyclic task (runs at fixed intervals)
...
class SubTask(BaseModel):
    ...
    agent_type: str = "default"
    # Use Field(default_factory=dict) so each instance gets its own dict
    ...
    occurrence_count: int = 0  # Number of times executed
...
class TaskCenter:
    ...
    async def create_task(
        ...
        platforms: List[str] = None # New parameter
    ) -> SubTask:
        """创建新任务"""
        async with self._lock:
            ...
            task = SubTask(
                ...
                context=context or {},
                platforms=platforms or [] # Assignment
            )
