...
# Key imports: must include all relevant Pydantic models
...
class SubAgentExecutor:
        ...
        try:
            async with httpx.AsyncClient(timeout=600.0) as http_client:
                while iteration < max_iterations:
                    ...
                    latest_task = await task_center.get_task(task_id)
                    # Check whether it was completed via a tool (at which point it won't be accidentally overwritten again)
                    ...
                    # Intelligently check the completion status
    ...
    async def _finalize_task_record(self, task_id, task_center, result, history, iteration):
        ...
        results_history = results_history[-20:] # Keep only the most recent 20
        ...
        # B. Push to external platforms (Telegram, Discord, etc.)
    ...
    async def _call_llm_stream_only(
                ...
                async for line in response.aiter_lines():
                    ...
                    try:
                        ...
                        if now - last_update_time >= UPDATE_INTERVAL:
                            ...
                            # Core fix 1: drop status=TaskStatus.RUNNING or change to status=None, to avoid wiping an already-triggered completed status
                            await task_center.update_task_progress(
        ...
        # Core fix 2: likewise remove the trailing logic that force-overwrites the status
        await task_center.update_task_progress(
