...
def parse_github_url(url: str):
    ...
    # Regex matching owner, repo, and possibly tree/branch/path
    ...
    # If the URL doesn't specify a branch, fetch the default branch dynamically
    if not branch:
        import httpx
        try:
            ...
            with httpx.Client(timeout=5.0) as client:
                ...
                if response.status_code == 200:
                    ...
                    branch = "main"  # Fallback strategy
        except Exception:
            branch = "main"  # Fallback strategy when the API call fails
    else:
        branch = branch
...
def get_skill_metadata(skill_dir: Path, skill_id: str) -> Skill:
    ...
    # 3. Find the metadata file (case-insensitive, supporting more variants)
    target_files = [
    ...
    try:
        # Use a generator to avoid instantiating all paths up front
        meta_file = next(
    ...
    if meta_file is not None:
        try:
            # Check the file size to avoid memory issues from reading huge files
            ...
            if file_size > 1024 * 1024:  # 1MB limit
                ...
                if content is not None:
                    # Extract the YAML between --- markers (looser matching)
                    match = re.search(
                        r'^\s*---\s*[\r\n]+(.*?)[\r\n]+---\s*',
                        content,
...
def _install_skills_from_directory(source_dir: Path) -> List[str]:
    ...
    # 1. Check whether source_dir is itself a skill
    ...
    def find_skill_dirs(root: Path):
        ...
        if is_skill_dir(root):
            ...
            # Scan subdirectories (ignoring hidden folders)
            try:
    ...
    # 4. Install the found skills, handling ID conflicts
    used_ids = set()
    for skill_dir in found_skill_dirs:
        # Prefer the directory name as the ID
        skill_id = skill_dir.name
        if skill_id in used_ids:
            # Create a unique ID using the path relative to search_dir
            try:
...
async def _process_github_install(url: str) -> Dict[str, Any]:
    ...
    try:
        ...
        # 3. Locate the content root (the top level of a GitHub ZIP is usually repo-main)
        repo_root = next(extract_dir.iterdir())
        
        # 4. If a subpath is given, descend into it
        ...
        # 5. Call the unified installer (now supports deep recursion)
...
async def upload_skill_zip(file: UploadFile = File(...)):
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        # Handle a possible 'wrapped' directory structure
