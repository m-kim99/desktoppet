...
아티팩트(claude.ai) 원본을 그대로 저장한 것 — 다른 세션·다른 모델에서도 이 파일만 열면 기준 확인 가능.

- `fruit-bite-spec.html` — 과일 11종 (초기/1입/2입)
- `food-bite-spec.html` — 푸드 부스 9종 (온전/1입/2입)
- `drink-sip-spec.html` — 음료 9종 (가득/절반/조금)
- `harvest-dish-spec.html` — 수확 요리 8종 (온전/1입/2입)
- `space-snack-spec.html` — 우주 자판기 5종 (현재/보완/1입/2입)

## 3D 검수 랩 (world.js에 내장, URL 파라미터로 활성화 — 커밋에는 미포함)
- `world.html?foodlab=1` — 음식 9종 × 베어물기 3단계
- `world.html?drinklab=1` — 음료 9종 × 마시기 3단계
- `world.html?snacklab=1` — 우주식 5종 × 베어물기 3단계
- `world.html?dishlab=1` — 수확 요리 8종 × 먹기 3단계
- 헤드리스 스크린샷: `scripts/_foodlab-shot.mjs` 등 (`_` 접두사 = 임시)
